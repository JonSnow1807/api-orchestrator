...
const WorkspaceSwitcher = ({ onWorkspaceChange }) => {
  ...
  const fetchWorkspaces = async () => {
    try {
      ...
      // If API fails, use mock data for UI
      const mockWorkspaces = [
        { id: 1, name: 'Personal Workspace', role: 'owner', member_count: 1 },
      ];
      setWorkspaces(mockWorkspaces);
      setCurrentWorkspace(mockWorkspaces[0]);
      onWorkspaceChange?.(mockWorkspaces[0]);
    }
  };
  ...
  return (
    <>
      ...
      {showCreateModal && ReactDOM.createPortal(
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center" style={{ zIndex: 9999999 }}>
