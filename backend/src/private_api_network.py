...
class PrivateAPINetwork:
    ...
    async def register_api(
        ...
        for existing_api in self.apis.values():
            if existing_api.name == api_metadata.name and existing_api.id != api_metadata.id:
    ...
    async def track_api_usage(
        ...
        api.avg_response_time_30d = (
            ...
            (1 - alpha) * api.avg_response_time_30d
        )
    ...
    async def generate_api_report(
        ...
        if include_health:
            ...
            report["health_summary"] = {
                ...
                "warning": warning,
                "critical": critical,
                "avg_response_time": sum(api.avg_response_time_30d for api in apis) / len(apis) if apis else 0,
                "avg_error_rate": sum(api.error_rate_30d for api in apis) / len(apis) if apis else 0
            }
        ...
        if include_usage:
            report["usage_summary"] = {
                ...
                "most_used_apis": sorted(
                    [{"name": api.name, "requests": api.total_requests_30d} for api in apis],
        ...
        if include_compliance:
            ...
            report["compliance_summary"] = {
                ...
                "apis_without_compliance": len([api for api in apis if not api.compliance_tags]),
                "apis_needing_security_scan": len([
                    api for api in apis 
                    if not api.last_security_scan or 
                    (datetime.now() - api.last_security_scan).days > 30
                ])
            }
