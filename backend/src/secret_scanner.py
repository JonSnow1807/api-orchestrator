...
class SecretScanner:
    ...
    async def scan_content(
        ...
        scan_result = SecretScanResult(
            scan_id=hashlib.sha256(f"{workspace_id}{datetime.now()}".encode()).hexdigest()[:16],
    ...
    async def scan_workspace(
        ...
        if not exclude_patterns:
            exclude_patterns = ['*.pyc', '__pycache__', '.git', 'node_modules', '.env.example']
        ...
        for root, dirs, files in os.walk(workspace_path):
            ...
            for file in files:
                ...
                try:
                    ...
                    # Scan content
                    file_result = await self.scan_content(content, file_path, workspace_id)
                    ...
                    scan_result.scan_errors.append(f"Error scanning {file_path}: {str(e)}")
        ...
        try:
            if provider == VaultProvider.HASHICORP:
                ...
                client.secrets.kv.v2.create_or_update_secret(
                    path=path,
                    secret={"value": secret_value, **metadata} if metadata else {"value": secret_value}
                )
                result["success"] = True
                ...
                response = client.create_secret(
                    ...
                    Tags=[{"Key": k, "Value": str(v)} for k, v in metadata.items()] if metadata else []
                )
                result["success"] = True
                ...
                result["success"] = True
                result["secret_path"] = f"{config.connection_string}/secrets/{secret_name}"
    ...
    def _generate_recommendations(self, scan_result: SecretScanResult) -> List[str]:
        ...
        if scan_result.total_secrets_found == 0:
            recommendations.append("✅ No secrets detected. Good job maintaining security!")
            return recommendations
        
        # Critical secrets
        critical_count = scan_result.secrets_by_severity.get(SecretSeverity.CRITICAL.value, 0)
        if critical_count > 0:
        ...
        # Private Keys
        private_key_count = scan_result.secrets_by_type.get(SecretType.PRIVATE_KEY.value, 0)
