...
class TrafficMonitor:
    ...
    async def record_metric(self, metric: TrafficMetric):
        ...
        # Notify subscribers
        await self._notify_subscribers({"type": "metric", "data": metric.to_dict()})
    ...
    async def get_endpoint_stats(self, top_n: int = 10) -> List[Dict[str, Any]]:
        ...
        for endpoint, count in sorted(
            ...
            endpoint_metrics = [
                m for m in self.metrics if f"{m.method} {m.endpoint}" == endpoint
            ]
    ...
    async def get_time_series_data(
        ...
        for minute_key in sorted(self.time_series_data.keys()):
            ...
            if minute_time > cutoff_time:
                data_point = {"timestamp": minute_key, "value": 0}

                if metric_type == MetricType.REQUESTS_PER_SECOND:
                    data_point["value"] = (
                        len(self.time_series_data[minute_key]["metrics"]) / 60
                    )
                    ...
                    data_point["value"] = self.time_series_data[minute_key].get(
                        "bandwidth", 0
                    )
    ...
    async def _check_alerts(self, metric: TrafficMetric):
        ...
        if (
            recent_stats.requests_per_second
            > self.alert_thresholds["requests_per_second"]
        ):
            alerts.append(
                {
                    "type": "rate_limit",
        ...
        for alert in alerts:
            ...
            await self._notify_subscribers({"type": "alert", "alert": alert})
    ...
    async def _aggregate_metrics_task(self):
        ...
        while self.running:
            ...
            if self.current_minute_metrics:
                ...
                response_times = [
                    m.response_time_ms for m in self.current_minute_metrics
                ]
                if response_times:
                ...
                await self._notify_subscribers(
                    {"type": "stats_update", "stats": stats.to_dict()}
                )
