...
class NaturalLanguageTestGenerator:
    ...
    def generate_test(self, natural_language: str, context: Optional[Dict] = None) -> Dict[str, Any]:
        """
            ...
            if match:
                if callable(generator):
                    ...
                    if test_code:
                        ...
                        test_descriptions.append(self._get_description_for_test(pattern, match))
        
        # If no patterns matched, try intelligent generation
        if not generated_tests and context:
            generated_tests = self._intelligent_test_generation(natural_language, context)
    ...
    def _generate_property_value_test(self, property_name: str, expected_value: str) -> str:
...
pm.expect(dateValue.getTime()).to.be.below(now.getTime())"""
    ...
    def _intelligent_test_generation(self, description: str, context: Dict) -> List[str]:
            ...
            if isinstance(response, dict):
                ...
                for key, value in response.items():
                    if isinstance(value, list):
                        tests.append(f"pm.expect(pm.response.json().{key}).to.be.an('array')")
                    elif isinstance(value, dict):
                        tests.append(f"pm.expect(pm.response.json().{key}).to.be.an('object')")
    ...
    def _get_suggestions(self, input_text: str) -> List[str]:
        ...
        elif any(word in input_text for word in ['fast', 'slow', 'time', 'performance']):
    ...
    async def generate_from_response(self, response_data: Dict, selected_field: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if selected_field:
            ...
            if field_value is not None:
                ...
                if isinstance(field_value, bool):
                    tests.append(f"pm.expect(pm.response.json().{selected_field}).to.be.a('boolean')")
                    tests.append(f"pm.expect(pm.response.json().{selected_field}).to.eql({str(field_value).lower()})")
                elif isinstance(field_value, int) or isinstance(field_value, float):
                    tests.append(f"pm.expect(pm.response.json().{selected_field}).to.be.a('number')")
                    tests.append(f"pm.expect(pm.response.json().{selected_field}).to.eql({field_value})")
                elif isinstance(field_value, str):
                    tests.append(f"pm.expect(pm.response.json().{selected_field}).to.be.a('string')")
                    if '@' in field_value:  # Likely email
                        tests.append(f"pm.expect(pm.response.json().{selected_field}).to.match(/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/)")
                    elif field_value.startswith('http'):  # Likely URL
                        tests.append(f"pm.expect(pm.response.json().{selected_field}).to.match(/^https?:\\/\\/.+/)")
                    else:
                        tests.append(f"pm.expect(pm.response.json().{selected_field}).to.eql('{field_value}')")
                elif isinstance(field_value, list):
                    tests.append(f"pm.expect(pm.response.json().{selected_field}).to.be.an('array')")
                    tests.append(f"pm.expect(pm.response.json().{selected_field}).to.have.lengthOf({len(field_value)})")
                elif isinstance(field_value, dict):
                    tests.append(f"pm.expect(pm.response.json().{selected_field}).to.be.an('object')")
                    for key in field_value.keys():
                        tests.append(f"pm.expect(pm.response.json().{selected_field}).to.have.property('{key}')")
        else:
        ...
        if tests:
            descriptions = [f"Test for {selected_field}" if selected_field else "Comprehensive response test"] * len(tests)
    ...
    def _analyze_response_structure(self, data: Any, prefix: str = "") -> List[str]:
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                tests.append(f"pm.expect(pm.response.json(){('.' + prefix) if prefix else ''}).to.have.property('{key}')")
                
                # Type checks
                if isinstance(value, bool):
                    tests.append(f"pm.expect(pm.response.json().{field_path}).to.be.a('boolean')")
                elif isinstance(value, (int, float)):
                    tests.append(f"pm.expect(pm.response.json().{field_path}).to.be.a('number')")
                elif isinstance(value, str):
                    tests.append(f"pm.expect(pm.response.json().{field_path}).to.be.a('string')")
                elif isinstance(value, list):
                    tests.append(f"pm.expect(pm.response.json().{field_path}).to.be.an('array')")
                elif isinstance(value, dict):
                    tests.append(f"pm.expect(pm.response.json().{field_path}).to.be.an('object')")
