...
from fastapi import Request, Response, Depends, HTTPException, BackgroundTasks
from fastapi.middleware.base import BaseHTTPMiddleware
from sqlalchemy import Column, String, Integer, DateTime, Text, JSON, Boolean, Index, func
...
class EnterpriseAuditManager:
    ...
    async def log_event(
        ...
        if request:
            request_info = {
                ...
                'body_hash': self._hash_request_body(request) if hasattr(request, 'body') else None
            }
    ...
    def _hash_request_body(self, request: Request) -> Optional[str]:
        ...
        try:
            if hasattr(request, 'body'):
                body_bytes = request.body if isinstance(request.body, bytes) else str(request.body).encode()
    ...
    def _should_trigger_alert(self, alert: AuditAlert, audit_entry: AuditLog, db: Session) -> bool:
        ...
        if (severity_levels.get(audit_entry.severity, 0) <
            severity_levels.get(alert.severity_threshold, 0)):
            return False

        # Check frequency threshold
        if alert.frequency_threshold:
            time_window = datetime.utcnow() - timedelta(minutes=alert.time_window_minutes)
    ...
    async def _trigger_alert(self, alert: AuditAlert, audit_entry: AuditLog):
        ...
        if alert.email_recipients:
            asyncio.create_task(self._send_email_alert(alert.email_recipients, alert_data))
    ...
    async def _send_email_alert(self, recipients: List[str], alert_data: Dict[str, Any]):
        ...
        self.logger.info(f"Email alert sent to {recipients}: {alert_data['alert_name']}")
        ...
        slack_message = {
            ...
            "attachments": [{
                ...
                "fields": [
                    ...
                    {"title": "User", "value": alert_data['user_email'], "short": True},
                    {"title": "IP Address", "value": alert_data['ip_address'], "short": True},
            ...
            'audit_logs': [
                {
                    ...
                    'location': f"{log.city}, {log.region}, {log.country}" if log.city else None,
                    'details': log.details
                }
                for log in logs
            ]
