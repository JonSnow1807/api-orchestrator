...
class ProxySettings(BaseModel):
    ...
    presets: List[Dict[str, Any]] = Field(default_factory=lambda: [
        {
            "name": "Local Proxy",
            "type": "http",
            "host": "localhost",
...
async def test_proxy_connection(config: ProxyConfig, test_url: str = "http://httpbin.org/ip") -> ProxyTestResult:
    ...
    try:
        ...
        async with httpx.AsyncClient(proxies=proxies, verify=config.verify_ssl, timeout=10) as client:
