...
from src.team_collaboration_models import (
    Workspace, WorkspaceMember, WorkspaceRole, ResourcePermission,
    ROLE_PERMISSIONS
)
...
class PermissionChecker:
    ...
    def has_workspace_permission(self, user_id: int, workspace_id: int, 
    ...
    def has_resource_permission(self, user_id: int, workspace_id: int,
                              resource_type: str, resource_id: str, action: str) -> bool:
        """
        ...
        return self.has_workspace_permission(user_id, workspace_id, 
                                           f"{resource_type}s", action)
    ...
    def get_user_permissions_summary(self, user_id: int, workspace_id: int) -> Dict[str, Any]:
...
def require_resource_permission(resource_type: str, action: str):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not db or not current_user or workspace_id is None or resource_id is None:
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
class RoleManager:
    ...
    def create_custom_role(self, workspace_id: int, name: str, display_name: str,
                          description: str, permissions: Dict[str, Any], 
                          color: str = '#6B7280') -> WorkspaceRole:
        """Create a custom role for workspace"""
        # Check if role name already exists
        existing = self.db.query(WorkspaceRole).filter(
    ...
    def update_role_permissions(self, role_id: int, permissions: Dict[str, Any]) -> WorkspaceRole:
    ...
    def delete_role(self, role_id: int) -> bool:
        ...
        members_with_role = self.db.query(WorkspaceMember).filter(
    ...
    def assign_role(self, user_id: int, workspace_id: int, role_name: str) -> WorkspaceMember:
        ...
        custom_roles = self.db.query(WorkspaceRole).filter(
            WorkspaceRole.workspace_id == workspace_id
        ).all()
...
class SharingManager:
    ...
    def share_resource(self, workspace_id: int, resource_type: str, resource_id: str,
                      target_user_id: int, permissions: Dict[str, bool], 
                      granted_by_user_id: int) -> ResourcePermission:
    ...
    def revoke_resource_access(self, workspace_id: int, resource_type: str, 
                              resource_id: str, target_user_id: int) -> bool:
    ...
    def get_resource_permissions(self, workspace_id: int, resource_type: str, 
                               resource_id: str) -> List[ResourcePermission]:
    ...
    def get_user_shared_resources(self, user_id: int, workspace_id: int) -> List[Dict[str, Any]]:
        """Get all resources shared with user"""
        permissions = self.db.query(ResourcePermission).filter(
            ResourcePermission.workspace_id == workspace_id,
            ResourcePermission.user_id == user_id
        ).all()
