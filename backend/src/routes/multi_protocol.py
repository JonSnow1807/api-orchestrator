    ...
    try:
        ...
        if request.save_result and request.project_id:
            test_config = MultiProtocolTest(
                ...
                config={"headers": request.headers, "timeout": request.timeout},
        ...
        return {"success": len(result.get("errors", [])) == 0, "result": result}
...
async def test_soap(
    ...
    try:
        ...
        return {
            "success": result.get("soap_fault") is None
...
async def test_grpc(
    ...
    try:
        ...
        return {
            "success": result.get("grpc_status") == 0
            or result.get("grpc_status") is None,
            "result": result,
        }
...
async def get_multi_protocol_test(
    ...
    test = db.query(MultiProtocolTest).filter(MultiProtocolTest.id == test_id).first()
...
async def rerun_multi_protocol_test(
    ...
    if test.protocol == ProtocolType.WEBSOCKET:
        ...
        raise HTTPException(
            status_code=400, detail=f"Unknown protocol: {test.protocol}"
        )
...
async def get_websocket_sample_messages(endpoint_type: str = "echo"):
    """Get sample WebSocket messages for testing"""

    messages = WebSocketTester.generate_test_messages(endpoint_type)

    return {"success": True, "endpoint_type": endpoint_type, "messages": messages}


@router.post("/grpc/discover")
async def discover_grpc_services(url: str, current_user=Depends(get_current_user)):
