...
async def transform_data_for_chart(
    ...
    try:
        ...
        transformed = data_viz_engine.transform_for_visualization(data, viz_enum, options)
...
async def save_collection_offline(
    ...
    try:
        ...
        file_path = await offline_manager.save_collection(
            ...
            format=StorageFormat[format_type],
            path=path
        )
...
async def watch_offline_directory(
    ...
    try:
        ...
        await offline_manager.watch_directory(
            directory=directory,
            callback=lambda path: print(f"File changed: {path}")
        )
...
async def discover_apis_for_vscode(
    ...
    try:
        ...
        file_patterns = request.get("patterns", ["**/*.py", "**/*.js", "**/*.ts"])
