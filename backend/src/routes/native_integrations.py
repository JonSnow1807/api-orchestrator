...
async def list_integrations(
    ...
    try:
        ...
        user_integrations = await integrations.list_user_integrations(
            user_id=current_user.id, enabled_only=enabled_only
        )

        return {"integrations": user_integrations, "total": len(user_integrations)}
    except ImportError:
        return {
            "integrations": [
                {
...
async def send_slack_message(
    ...
    try:
        ...
        return {"channel": message.channel, "timestamp": message_ts, "status": "sent"}
    except ImportError:
        return {
            "channel": message.channel,
...
async def send_datadog_metric(
    ...
    try:
        ...
        await datadog.send_metric(
            metric_name=metric_name, value=value, tags=tags, metric_type=metric_type
        )

        return {"metric": metric_name, "value": value, "tags": tags, "status": "sent"}
    except ImportError:
        return {
            "metric": metric_name,
            "value": value,
...
async def create_pagerduty_incident(
    ...
    try:
        ...
        incident_id = await pagerduty.create_incident(
            title=title, description=description, urgency=urgency, service_id=service_id
        )
        ...
        "triggers": [
            {
                ...
                "config": {"urgency": "high", "service": "api-service"},
                "enabled": True,
            ...
            },
            {
                ...
                "integration": "slack",
                "action": "send_alert",
                "config": {"channel": "#api-alerts", "mention": "@oncall"},
                "enabled": True,
                "created_at": datetime.utcnow().isoformat(),
            },
        ],
...
async def get_integration_status(
    ...
    return {
        "integrations": {
            ...
            "teams": {"connected": False, "error": "Webhook URL not configured"},
            "github": {
                "connected": True,
                "repos_connected": 3,
                "last_activity": datetime.utcnow().isoformat(),
            },
            "datadog": {"connected": True, "metrics_sent_today": 1542},
            "pagerduty": {
                "connected": True,
                "open_incidents": 0,
                "on_call": "team-alpha",
