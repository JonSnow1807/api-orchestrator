...
async def search_request_history(
    ...
    offset: int = Query(0),
    sort_by: str = Query(
        "created_at", regex="^(created_at|response_time_ms|status_code|method)$"
    ),
...
async def get_request_stats(
    ...
    for req in requests:
        ...
        # Count status codes
        if req.status_code:
            status_counts[str(req.status_code)] = (
                status_counts.get(str(req.status_code), 0) + 1
            )
    ...
    return {
        ...
        "top_endpoints": [{"url": url, "count": count} for url, count in top_endpoints],
    }
...
async def delete_request_history(
    ...
    request = (
        db.query(RequestHistory)
        .filter(
            RequestHistory.id == request_id, RequestHistory.user_id == current_user.id
        )
        .first()
    )
