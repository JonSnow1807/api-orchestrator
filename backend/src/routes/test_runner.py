    ...
    if request.test_id:
        ...
        if test:
            test_result = TestResult(
                ...
                assertions_failed=sum(1 for a in result.get('assertions', []) if not a['passed']),
...
async def run_test_suite(
    ...
    if request.suite_id:
        suite_model = db.query(TestSuite).filter(TestSuite.id == request.suite_id).first()
        if not suite_model:
    ...
    # Create suite
    suite = TestSuite(
        name=f"Suite {request.suite_id or 'Ad-hoc'}",
        tests=test_cases
    )
...
async def get_suite_history(
    ...
    # Get suite results
    results = db.query(TestSuiteResult).filter(
        TestSuiteResult.suite_id == suite_id
    ).order_by(TestSuiteResult.completed_at.desc()).limit(limit).all()
        ...
        "types": [
            {
                ...
                "example": {"type": "STATUS_CODE", "expected": 200, "operator": "equals"}
            },
            {
                "type": "RESPONSE_TIME",
                "description": "Check response time in milliseconds",
                "operators": ["less_than", "greater_than", "equals"],
                "example": {"type": "RESPONSE_TIME", "expected": 1000, "operator": "less_than"}
            },
            {
                "type": "BODY_CONTAINS",
                "description": "Check if response body contains text",
                "operators": ["contains"],
                "example": {"type": "BODY_CONTAINS", "expected": "success", "operator": "contains"}
            },
            {
                "type": "BODY_JSON_PATH",
                "description": "Check JSON path value",
                "operators": ["equals", "not_equals", "exists"],
                "example": {"type": "BODY_JSON_PATH", "expected": {"path": "$.user.name", "value": "John"}}
            },
            {
                "type": "HEADER_EXISTS",
            ...
            },
            {
                ...
                "example": {"type": "HEADER_VALUE", "expected": {"name": "Content-Type", "value": "application/json"}}
            },
            {
                "type": "BODY_REGEX",
