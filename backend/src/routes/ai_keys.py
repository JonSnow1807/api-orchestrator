...
if not ENCRYPTION_KEY:
    ...
    print(
        f"Warning: Generated new encryption key. Set AI_KEY_ENCRYPTION_KEY env var in production."
    )
...
def verify_workspace_access(
    workspace_id: int, user: User, db: Session, required_role: str = "admin"
):
...
async def create_ai_key(
    ...
    workspace = verify_workspace_access(
        key_data.workspace_id, current_user, db, "admin"
    )

    # Validate provider
    if key_data.provider not in AI_PROVIDERS:
        raise HTTPException(
            status_code=400, detail=f"Invalid provider: {key_data.provider}"
        )
    ...
    existing = (
        db.query(AIKey)
        .filter(AIKey.key_hash == key_hash, AIKey.workspace_id == key_data.workspace_id)
        .first()
    )
    if existing:
        raise HTTPException(
            status_code=400, detail="This API key already exists in the workspace"
        )
    ...
    usage = (
        db.query(AIKeyUsage)
        .filter(AIKeyUsage.key_id == ai_key.id, AIKeyUsage.date >= current_month)
        .all()
    )
...
async def list_ai_keys(
    ...
    for key in keys:
        ...
        usage = (
            db.query(AIKeyUsage)
            .filter(AIKeyUsage.key_id == key.id, AIKeyUsage.date >= current_month)
            .all()
        )
...
async def get_ai_key_usage(
    ...
    usage_data = (
        db.query(AIKeyUsage)
        .filter(AIKeyUsage.key_id == key_id, AIKeyUsage.date >= start_date)
        .order_by(AIKeyUsage.date)
        .all()
    )
    ...
    for usage in usage_data:
        if usage.model:
            model_breakdown[usage.model] = (
                model_breakdown.get(usage.model, 0) + usage.requests
            )
...
async def test_ai_key(
    ...
    try:
        if key.provider == "openai":
            ...
            test_result[
                "message"
            ] = f"Testing for {key.provider} provider is not yet implemented"
