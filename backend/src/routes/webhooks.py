...
def verify_workspace_access(workspace_id: int, user: User, db: Session, required_role: str = "developer"):
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code < 300:
                    ...
                    if webhook.failure_count >= 10:
                        webhook.is_active = False  # Disable after 10 consecutive failures
                    return {
                        "delivered": False,
                        "status_code": response.status_code,
                        "response_time_ms": duration_ms,
...
async def create_webhook(
    ...
    workspace = verify_workspace_access(webhook_data.workspace_id, current_user, db, "admin")
...
async def list_webhooks(
    ...
    query = db.query(WorkspaceWebhook).filter(WorkspaceWebhook.workspace_id == workspace_id)
...
async def get_webhook(
    ...
    webhook = db.query(WorkspaceWebhook).filter(WorkspaceWebhook.id == webhook_id).first()
    if not webhook:
...
async def trigger_webhooks(db: Session, workspace_id: int, event: str, data: Dict[str, Any]):
    """Trigger all active webhooks for a workspace and event"""
    webhooks = db.query(WorkspaceWebhook).filter(
