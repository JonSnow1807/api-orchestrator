...
async def create_public_docs(
    ...
    try:
        ...
        existing_docs = (
            db.query(PublicDocumentation)
            .filter(PublicDocumentation.project_id == docs_request.project_id)
            .first()
        )
        ...
        if existing_docs:
            ...
            existing_docs.base_url = (
                str(docs_request.base_url) if docs_request.base_url else None
            )
            existing_docs.theme = docs_request.theme
            existing_docs.logo_url = (
                str(docs_request.logo_url) if docs_request.logo_url else None
            )
...
async def get_public_docs(
    ...
    docs = (
        db.query(PublicDocumentation)
        .filter(PublicDocumentation.project_id == project_id)
        .first()
    )

    if not docs:
        raise HTTPException(status_code=404, detail="Public documentation not found")
...
async def serve_public_docs(
    subdomain: str, request: Request, db: Session = Depends(get_db)
):
    ...
    if docs.custom_domain and host != docs.custom_domain:
        ...
        return RedirectResponse(url=f"https://{docs.custom_domain}", status_code=301)
...
async def serve_openapi_spec(subdomain: str, db: Session = Depends(get_db)):
...
async def get_docs_analytics(
    ...
    docs = (
        db.query(PublicDocumentation)
        .filter(PublicDocumentation.subdomain == subdomain)
        .first()
    )
    ...
    project = (
        db.query(Project)
        .filter(Project.id == docs.project_id, Project.user_id == current_user.id)
        .first()
    )
    ...
    try:
        ...
        if docs.openapi_spec:
            (docs_dir / "openapi.json").write_text(
                json.dumps(docs.openapi_spec, indent=2), encoding="utf-8"
            )
