...
async def create_status_page(
    ...
    async def start_monitoring():
        await health_monitor.start_monitoring(
            monitor_id=monitor_id, endpoints=request.services, interval_seconds=60
        )
...
async def get_status_page_config(
    page_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
...
async def get_status_page_health(page_id: int, db: Session = Depends(get_db)):
        ...
        for service in page.services:
            ...
            if health:
                ...
                if health.status == ServiceStatus.MAJOR_OUTAGE:
                    ...
                    health.status == ServiceStatus.PARTIAL_OUTAGE
                    and overall_status != ServiceStatus.MAJOR_OUTAGE
                ):
                    overall_status = ServiceStatus.PARTIAL_OUTAGE
                elif (
                    health.status == ServiceStatus.DEGRADED
                    and overall_status == ServiceStatus.OPERATIONAL
                ):
...
async def update_status_page(
    ...
    return {"success": True, "message": "Status page updated successfully"}
...
async def get_public_status_page(slug: str, db: Session = Depends(get_db)):
    ...
    page = (
        db.query(StatusPage)
        .filter(StatusPage.slug == slug, StatusPage.is_public == True)
        .first()
    )
    ...
    return {
        ...
        "page": {
            ...
            "overall_status": overall_status.value,
            "services": services_health,
            "uptime_history": health_monitor.get_uptime_history(monitor_id, 90)
            if page.show_uptime_history
            else None,
            "incidents": health_monitor.get_incident_timeline(monitor_id)
            if page.show_incident_timeline
            else None,
        },
    }
