...
from src.graphql_handler import (
    GraphQLQuery, GraphQLSchema, GraphQLQueryBuilder,
    GraphQLIntrospector, GraphQLExecutor, GraphQLTestGenerator
)
...
async def get_graphql_schema(
    ...
    schema = db.query(GraphQLSchema).filter(
        GraphQLSchema.id == schema_id
    ).first()
...
async def generate_graphql_tests(
    ...
    schema = db.query(GraphQLSchema).filter(
        GraphQLSchema.id == request.schema_id
    ).first()
...
async def suggest_graphql_query(
    ...
    operation = next((op for op in operation_list if op["name"] == operation_name), None)
