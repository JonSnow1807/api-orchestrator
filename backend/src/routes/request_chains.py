...
async def get_request_chain(
    chain_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
    """Get details of a specific request chain"""

    chain = (
        db.query(RequestChain)
        .filter(RequestChain.id == chain_id, RequestChain.user_id == current_user.id)
        .first()
    )
...
async def update_request_chain(
    ...
    return {"success": True, "message": "Chain updated successfully"}
...
async def duplicate_request_chain(
    ...
    return {"success": True, "chain": {"id": new_chain.id, "name": new_chain.name}}
...
async def test_value_extraction(response_data: Dict[str, Any], extraction_path: str):
