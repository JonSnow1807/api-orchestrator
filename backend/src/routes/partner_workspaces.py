...
async def revoke_partner_access(
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Access revoked for partner {partner_id}"
        }
...
async def update_partner_workspace_settings(
    ...
    try:
        ...
        # Update settings
        updateable_fields = [
            'name', 'description', 'require_nda', 'watermark_enabled',
            'download_disabled', 'copy_disabled', 'ip_whitelist',
            'allowed_domains', 'max_partners', 'branding', 'welcome_message'
        ]
