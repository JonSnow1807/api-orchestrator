...
async def get_proxy_settings(
    ...
    return {
        "success": True,
        "settings": {
            "global_proxy": proxy_manager.global_config.dict()
            if proxy_manager.global_config
            else None,
            "workspace_proxies": {
                key: config.dict()
...
async def set_global_proxy(
    ...
    if request.username and request.password:
        config.auth = ProxyAuth(username=request.username, password=request.password)
...
async def test_proxy(
    ...
    if request.config.username and request.config.password:
        config.auth = ProxyAuth(
            username=request.config.username, password=request.config.password
        )
...
async def detect_system_proxy_settings(current_user=Depends(get_current_user)):
    """Detect system proxy settings"""

    system_proxy = detect_system_proxy()

    if system_proxy:
        return {"success": True, "detected": True, "proxy": system_proxy.dict()}
    else:
        return {
            "success": True,
            "detected": False,
...
async def delete_global_proxy(
    ...
    return {"success": True, "message": "Global proxy configuration deleted"}
...
async def delete_environment_proxy(
    env_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
...
@router.get("/presets")
async def get_proxy_presets(current_user=Depends(get_current_user)):
    ...
    presets = [
        ...
        },
        {
            ...
            "config": {"enabled": True, "use_system_proxy": True, "type": "system"},
        },
        {
