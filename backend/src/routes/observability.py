...
async def get_observability_dashboard(current_user: User = Depends(get_current_user)):
...
async def get_trace_timeline(
    ...
    total_duration = (
        max(item["end_offset_ms"] for item in timeline if item["end_offset_ms"])
        if timeline
        else 0
    )
    ...
    for span in observability.tracer.completed_spans:
        ...
        if span.duration_ms:
            service["avg_duration_ms"] = (
                service["avg_duration_ms"] * (service["request_count"] - 1)
                + span.duration_ms
...
async def add_health_check(
    request: HealthCheckRequest, current_user: User = Depends(get_current_user)
):
...
@router.get("/sli")
async def get_service_level_indicators(current_user: User = Depends(get_current_user)):
...
async def get_current_trace_info(current_user: User = Depends(get_current_user)):
...
async def add_trace_tag_endpoint(
    ...
    if not trace_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No active trace to tag"
        )
