...
class EnvironmentVariable(Base):
    ...
    scope = Column(String(50), default="workspace")  # global, workspace, collection, request
...
class EnvironmentManager:
    ...
    def create_environment(
        ...
        for key, value in variables.items():
            ...
            if any(word in key.lower() for word in ["password", "token", "key", "secret", "api_key"]):
                is_secret = True
        ...
        return {
            "name": name,
            "scope": scope,
            "variables": [EnvironmentManager._variable_to_dict(var) for var in environment_vars]
        }
    ...
    def resolve_variables(
        ...
        # 1. Global variables
        global_vars = db.query(EnvironmentVariable).filter(
        ...
        if workspace_id:
            workspace_vars = db.query(EnvironmentVariable).filter(
        ...
        if collection_id:
            collection_vars = db.query(EnvironmentVariable).filter(
    ...
    def get_environment_tree(
        ...
        tree["global"] = [EnvironmentManager._variable_to_dict(var) for var in global_vars]
        ...
        if workspace_id:
            ...
            tree["workspace"] = [EnvironmentManager._variable_to_dict(var) for var in workspace_vars]
