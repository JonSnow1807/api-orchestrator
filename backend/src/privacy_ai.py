...
class PrivacyFirstAI:
    ...
    def _initialize_local_model(self):
        ...
        try:
            ...
            logger.warning("Local AI not available, falling back to privacy-preserving cloud mode")
            self.local_model = None
    ...
    async def process_request(
        ...
        if self.policy.mode == AIMode.DISABLED:
            ...
            self.policy.mode == AIMode.HYBRID and 
            classification in [DataClassification.CONFIDENTIAL, DataClassification.RESTRICTED]
        ):
            result = await self._process_locally(anonymized_prompt, anonymized_data, context)
        else:
            result = await self._process_cloud_private(anonymized_prompt, anonymized_data, context)
    ...
    async def _anonymize_prompt(self, prompt: str, classification: DataClassification) -> str:
    ...
    def _is_processing_allowed(self, classification: DataClassification) -> bool:
        ...
        if classification == DataClassification.RESTRICTED:
            ...
            return self.policy.mode in [AIMode.LOCAL, AIMode.HYBRID] and self.local_model is not None
    ...
    def get_privacy_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "statistics": {
                ...
                "local_processing": sum(1 for log in self.audit_log if "local" in log.get("mode", "")),
                "cloud_processing": sum(1 for log in self.audit_log if "cloud" in log.get("mode", "")),
...
# Export for use
__all__ = ['PrivacyFirstAI', 'PrivacyPolicy', 'AIMode', 'DataClassification', 'get_privacy_ai']