...
class HealthMonitor:
    ...
    async def start_monitoring(
        self, monitor_id: str, endpoints: List[Dict], interval_seconds: int = 60
    ):
    ...
    def get_service_health(self, monitor_id: str) -> Optional[ServiceHealth]:
        ...
        uptime_percentage = (
            (healthy_count / len(uptime_checks) * 100) if uptime_checks else 0
        )
    ...
    def get_uptime_history(self, monitor_id: str, days: int = 90) -> List[Dict]:
        ...
        for day in range(days):
            ...
            day_checks = [c for c in checks if day_start <= c.timestamp < day_end]
