...
class PasswordValidator:
    ...
    def validate_password(self, password: str) -> tuple[bool, List[str]]:
        ...
        if len(password) < self.policy.min_password_length:
            errors.append(
                f"Password must be at least {self.policy.min_password_length} characters long"
            )
        ...
        if re.search(
            r"(.)\1{2,}", password
        ):  # Three or more consecutive same characters
...
class RateLimiter:
    ...
    def is_allowed(
        self, identifier: str, max_attempts: int = 100, window_minutes: int = 60
    ) -> bool:
...
class SecurityManager:
    ...
    def __init__(self, policy: SecurityPolicy = None):
        ...
        if encryption_key:
            ...
            logging.warning(
                "No ENCRYPTION_KEY environment variable set - using random key"
            )
    ...
    def authenticate_user(self, username: str, password: str, ip: str) -> Optional[str]:
        ...
        if not self.rate_limiter.is_allowed(
            f"login_{ip}", self.policy.max_login_attempts, 60
        ):
            self.audit_logger.log_security_event(
                "RATE_LIMIT_EXCEEDED", f"Login attempts for {username}", ip
            )
            return None
