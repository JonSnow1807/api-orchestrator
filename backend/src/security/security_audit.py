...
class SecurityAuditor:
    ...
    async def run_comprehensive_audit(self) -> SecurityAuditReport:
        ...
        self.logger.info(
            f"🔒 Security audit completed. Found {len(self.findings)} issues."
        )
        return report
    ...
    async def _scan_javascript_specific(self, file_path: Path, content: str):
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if "localStorage.setItem" in line and (
                "password" in line.lower() or "token" in line.lower()
            ):
                finding = SecurityFinding(
        ...
        for root, dirs, files in os.walk(self.project_root):
            ...
            for file in files:
                ...
                for pattern_name, pattern in config_patterns.items():
                    if file == pattern_name or file.endswith(
    ...
    async def _scan_dockerfile(self, dockerfile: Path):
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if line.startswith("ARG") and any(
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if VulnerabilityType.HARDCODED_SECRETS in vuln_types:
            recommendations.append(
                "Immediate action: Remove all hardcoded secrets from codebase"
            )

        if VulnerabilityType.SQL_INJECTION in vuln_types:
            recommendations.append(
                "Critical: Implement parameterized queries for all database operations"
            )
    ...
    def _check_compliance(self) -> Dict[str, bool]:
        ...
        critical_findings = [
            f for f in self.findings if f.severity == SecuritySeverity.CRITICAL
        ]
        high_findings = [
            f for f in self.findings if f.severity == SecuritySeverity.HIGH
        ]
...
async def run_security_audit():
    ...
    print(f"\n🚨 CRITICAL ISSUES:")
    critical_issues = [
        f for f in report.findings if f.severity == SecuritySeverity.CRITICAL
    ]
