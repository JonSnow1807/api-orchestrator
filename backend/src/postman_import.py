...
class PostmanImporter:
    ...
    def import_collection(self, collection_json: str) -> Dict[str, Any]:
        ...
        try:
            collection = json.loads(collection_json) if isinstance(collection_json, str) else collection_json
    ...
    def import_environment(self, environment_json: str) -> Dict[str, Any]:
        ...
        try:
            environment = json.loads(environment_json) if isinstance(environment_json, str) else environment_json
    ...
    def _import_auth(self, auth: Dict) -> Dict:
        ...
        if auth_type == 'bearer':
            ...
            username = next((item['value'] for item in basic if item.get('key') == 'username'), '')
            password = next((item['value'] for item in basic if item.get('key') == 'password'), '')
            return {
                "type": "basic",
                "username": username,
                "password": password
            ...
            key = next((item['value'] for item in apikey if item.get('key') == 'key'), '')
            value = next((item['value'] for item in apikey if item.get('key') == 'value'), '')
            location = next((item['value'] for item in apikey if item.get('key') == 'in'), 'header')
            return {
                "type": "apikey",
                "key": key,
                "value": value,
            ...
            return {
                "type": "oauth2",
                "access_token": next((item['value'] for item in oauth2 if item.get('key') == 'accessToken'), ''),
                "token_type": next((item['value'] for item in oauth2 if item.get('key') == 'tokenType'), 'Bearer')
            }
        else:
            return {"type": "noauth"}
    ...
    def convert_to_internal_format(self, imported_collection: Dict) -> Dict:
        ...
        internal = {
            ...
            "environments": [{
                "name": "Default",
                "variables": {var['key']: var['value'] for var in imported_collection.get('variables', [])}
            }],
            "requests": []
        }
        ...
        for request in imported_collection.get('requests', []):
            internal_request = {
                ...
                "body": request['body'],
                "auth": request['auth'],
                "assertions": self._convert_tests_to_assertions(request.get('tests', [])),
    ...
    def _convert_tests_to_assertions(self, test_scripts: List[str]) -> List[Dict]:
        ...
        for script in test_scripts:
            ...
            # Body contains check
            contains_match = re.findall(r'pm\.expect\(.*?responseBody.*?\)\.to\.include\(["\'](.+?)["\']\)', script)
            ...
            # JSON value check
            json_match = re.findall(r'pm\.expect\(jsonData\.(.+?)\)\.to\.eql\((.+?)\)', script)
            ...
            # Header exists
            header_match = re.findall(r'pm\.response\.to\.have\.header\(["\'](.+?)["\']\)', script)
