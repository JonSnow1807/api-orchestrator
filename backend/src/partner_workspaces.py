...
class PartnerWorkspaceManager:
    ...
    async def invite_partner(
        ...
        if len(workspace.partners) >= workspace.max_partners:
            raise ValueError(f"Maximum partner limit ({workspace.max_partners}) reached")
    ...
    async def accept_invite(
        ...
        access = PartnerAccess(
            ...
            ip_whitelist=workspace.ip_whitelist if workspace.access_level == AccessLevel.IP_RESTRICTED else None,
        ...
        workspace.pending_invites = [
            i for i in workspace.pending_invites 
            if i['token'] != token
        ]
    ...
    async def validate_access(
        ...
        if resource_type == "collection" and resource_id not in workspace.shared_collections:
            return False
    ...
    async def get_activity_report(
        ...
        report = {
            ...
            "modifications": len([log for log in logs if log.action in ["modify", "create", "delete"]]),
            "by_partner": {},
            "by_resource": {},
            "timeline": []
        }
        ...
        for partner in workspace.partners:
            ...
            report["by_partner"][partner.email] = {
                ...
                "last_activity": max([log.timestamp for log in partner_logs]).isoformat() if partner_logs else None
            }
        ...
        for log in sorted(logs, key=lambda x: x.timestamp, reverse=True)[:100]:
            report["timeline"].append({
                ...
                "partner": next((p.email for p in workspace.partners if p.partner_id == log.partner_id), "Unknown"),
                "action": log.action,
