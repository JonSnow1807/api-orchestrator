...
class Workspace(Base):
    ...
    activity_logs = relationship(
        "WorkspaceActivityLog", back_populates="workspace", cascade="all, delete-orphan"
    )
    active_sessions = relationship(
        "ActiveSession", back_populates="workspace", cascade="all, delete-orphan"
    )
    roles = relationship(
        "WorkspaceRole", back_populates="workspace", cascade="all, delete-orphan"
    )
        ...
        if include_stats:
            result["stats"] = {
                ...
                "pending_invitations": len(
                    [i for i in self.invitations if i.status == "pending"]
                ),
            }
    ...
    def get_member_by_user_id(self, user_id: int) -> Optional["WorkspaceMember"]:
        ...
        return next(
            (m for m in self.members if m.user_id == user_id and m.status == "active"),
            None,
        )
...
class WorkspaceRole(Base):
    ...
    # Indexes
    __table_args__ = (Index("idx_roles_workspace", "workspace_id"),)
...
class Collection(Base):
    ...
    shared_with = Column(
        JSON, default=lambda: []
    )  # Array of {user_id, permission} objects
    ...
    def can_user_access(
        self, user_id: int, workspace_member: Optional[WorkspaceMember]
    ) -> bool:
...
ROLE_PERMISSIONS = {
    "owner": {
        "workspace": {"read": True, "write": True, "delete": True, "billing": True},
        "members": {"invite": True, "remove": True, "manage_roles": True},
        "projects": {
            "create": True,
            "read": True,
            "write": True,
    ...
    "admin": {
        "workspace": {"read": True, "write": True, "delete": False, "billing": False},
        "members": {"invite": True, "remove": False, "manage_roles": True},
        "projects": {
            "create": True,
            "read": True,
            "write": True,
    ...
    "developer": {
        "workspace": {"read": True, "write": False, "delete": False, "billing": False},
