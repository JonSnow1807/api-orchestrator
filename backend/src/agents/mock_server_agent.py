...
class MockServerAgent:
    ...
    async def create_mock_server(self, api_spec: Dict, options: Dict = None) -> Dict:
        ...
        config = {
            ...
            "error_rate": options.get("error_rate", 0)
            if options
            else 0,  # Percentage of errors
            "stateful": options.get("stateful", True)
            if options
            else True,  # Remember state
        ...
        mock_dir = (
            Path("mock_servers") / f"mock_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
...
"""
    ...
    def _generate_endpoint_handler(
        self, path: str, method: str, details: Dict, stateful: bool
    ) -> str:
