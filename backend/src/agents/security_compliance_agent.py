...
class SecurityComplianceAgent:
    ...
    async def perform_security_scan(self, api_spec: Dict[str, Any], endpoints: List[str]) -> Dict[str, Any]:
        """Perform comprehensive security scan on API"""
        try:
            scan_id = hashlib.md5(f"{datetime.now().isoformat()}{json.dumps(api_spec)}".encode()).hexdigest()[:8]
            ...
            encryption_findings = await self._check_encryption_standards(api_spec, endpoints)
    ...
    async def check_compliance(self, api_spec: Dict[str, Any], frameworks: List[ComplianceFramework]) -> Dict[str, Any]:
        ...
        try:
            ...
            for framework in frameworks:
                ...
                compliance_score = self._calculate_compliance_score(framework_findings, len(rules))
            ...
            overall_score = sum(result["compliance_score"] for result in compliance_results.values()) / len(compliance_results) if compliance_results else 0
            ...
            return {
                ...
                "action_items": await self._get_compliance_action_items(compliance_results)
            }
    ...
    async def _check_authentication_security(self, api_spec: Dict[str, Any]) -> List[SecurityFinding]:
    ...
    async def _check_encryption_standards(self, api_spec: Dict[str, Any], endpoints: List[str]) -> List[SecurityFinding]:
        """Check encryption and TLS configuration"""
        findings = []

        # Check if HTTPS is enforced
        servers = api_spec.get("servers", [])
        has_https = any(server.get("url", "").startswith("https://") for server in servers)
        ...
        for endpoint in endpoints[:5]:  # Check first 5 endpoints to avoid overload
            try:
                ...
                if parsed.scheme == "https":
                    tls_issues = await self._check_tls_configuration(parsed.hostname, parsed.port or 443)
    ...
    async def _check_input_validation(self, api_spec: Dict[str, Any]) -> List[SecurityFinding]:
        ...
        for path, methods in paths.items():
            for method, operation in methods.items():
                ...
                for param in parameters:
                    if param.get("in") == "query" and not param.get("schema", {}).get("pattern"):
                        findings.append(SecurityFinding(
                            finding_id="VAL_002",
    ...
    async def _check_security_headers(self, endpoints: List[str]) -> List[SecurityFinding]:
    ...
    async def _check_access_controls(self, api_spec: Dict[str, Any]) -> List[SecurityFinding]:
    ...
    async def _check_owasp_api_security(self, api_spec: Dict[str, Any], endpoints: List[str]) -> List[SecurityFinding]:
    ...
    async def _check_tls_configuration(self, hostname: str, port: int) -> List[SecurityFinding]:
    ...
    async def _execute_compliance_check(self, rule: ComplianceRule, api_spec: Dict[str, Any]) -> Optional[SecurityFinding]:
        """Execute a specific compliance check"""
        # Simplified compliance check implementation
        if rule.check_function == "check_encryption":
            if not any(server.get("url", "").startswith("https://") for server in api_spec.get("servers", [])):
    ...
    def _calculate_compliance_score(self, violations: List[SecurityFinding], total_rules: int) -> float:
    ...
    async def _generate_security_recommendations(self, findings: List[SecurityFinding]) -> List[str]:
        ...
        if "Authentication" in categories:
            recommendations.append("🔐 Implement strong authentication mechanisms (OAuth 2.0, JWT)")
        ...
        if "Access Control" in categories:
            recommendations.append("🛡️ Establish proper access controls and authorization")
    ...
    async def _get_compliance_action_items(self, compliance_results: Dict[str, Any]) -> List[str]:
        ...
        for framework, result in compliance_results.items():
            if result["compliance_score"] < 80:
                action_items.append(f"📋 Address {framework.upper()} compliance violations ({result['violations']} issues)")
    ...
    async def get_security_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            recent_findings = [f for f in self.findings_history if f and hasattr(f, 'finding_id')][-50:]  # Last 50 findings

            dashboard = {
                "last_scan": self.last_scan_time.isoformat() if self.last_scan_time else None,
                "total_findings": len(self.findings_history),
                "recent_findings": len(recent_findings),
                "severity_distribution": self._group_findings_by_severity(recent_findings),
                ...
                "recommendations": await self._get_dashboard_recommendations(recent_findings)
            }
    ...
    def _get_top_categories(self, findings: List[SecurityFinding]) -> List[Dict[str, int]]:
        """Get top security categories by finding count"""
        category_counts = {}
        for finding in findings:
            category_counts[finding.category] = category_counts.get(finding.category, 0) + 1

        return sorted([{"category": k, "count": v} for k, v in category_counts.items()],
    ...
    async def _get_security_trends(self) -> List[Dict[str, Any]]:
        ...
        for i in range(7):
            ...
            day_findings = [f for f in self.findings_history
                          if hasattr(f, 'finding_id') and day.date() == day.date()]

            trends.append({
                "date": day.date().isoformat(),
                "total_findings": len(day_findings),
                "critical": len([f for f in day_findings if f.severity == SeverityLevel.CRITICAL]),
                "high": len([f for f in day_findings if f.severity == SeverityLevel.HIGH])
            })
    ...
    async def _get_dashboard_recommendations(self, findings: List[SecurityFinding]) -> List[str]:
        ...
        if high_count > 3:
            recommendations.append(f"⚡ {high_count} high-severity issues require attention")
...
if __name__ == "__main__":
    async def test_security_agent():
        ...
        sample_endpoints = ["http://api.example.com/users", "http://api.example.com/orders"]
        ...
        compliance_result = await agent.check_compliance(
            sample_spec,
            [ComplianceFramework.GDPR, ComplianceFramework.OWASP]
        )
