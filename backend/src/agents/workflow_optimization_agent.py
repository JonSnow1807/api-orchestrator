...
class WorkflowOptimizationAgent:
    ...
    async def analyze_user_workflow(self, user_id: str, actions: List[UserAction]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "recommendations": await self._get_recommendations(patterns, efficiency)
            }
    ...
    async def _detect_patterns(self, user_id: str) -> List[WorkflowPattern]:
        ...
        if testing_sequences:
            patterns.append(WorkflowPattern(
                ...
                average_time=statistics.mean([seq["duration"] for seq in testing_sequences]),
                steps=["Create request", "Set parameters", "Execute", "Validate response"],
        ...
        if collection_patterns:
            patterns.append(WorkflowPattern(
                ...
                average_time=120.0,
                steps=["Create collection", "Add requests", "Organize folders", "Share"],
    ...
    def _find_testing_sequences(self, actions: List[UserAction]) -> List[Dict]:
        ...
        for action in actions:
            if action.action_type in ["create_request", "execute_request", "validate_response"]:
                current_sequence.append(action)

                # If we have a complete sequence
                if len(current_sequence) >= 3 and action.action_type == "validate_response":
                    duration = (current_sequence[-1].timestamp - current_sequence[0].timestamp).total_seconds()
                    sequences.append({
    ...
    def _find_collection_patterns(self, actions: List[UserAction]) -> List[Dict]:
        ...
        for i, action in enumerate(collection_actions):
            ...
            for j in range(i + 1, len(collection_actions)):
                if (collection_actions[j].timestamp - action.timestamp).total_seconds() < 3600:
    ...
    def _find_environment_switching(self, actions: List[UserAction]) -> List[Dict]:
        ...
        for i in range(len(env_switches) - 1):
            time_diff = (env_switches[i + 1].timestamp - env_switches[i].timestamp).total_seconds()
            if time_diff < 600:  # Within 10 minutes
                switches.append({"actions": [env_switches[i], env_switches[i + 1]]})

        return switches

    async def _generate_optimizations(self, patterns: List[WorkflowPattern]) -> List[Dict]:
        ...
        for pattern in patterns:
            if pattern.optimization_potential > 0.7:
                optimizations.append({
                    ...
                    "implementation": pattern.suggested_improvements[0] if pattern.suggested_improvements else "Create automation",
                    "impact": "High"
                })
                ...
                optimizations.append({
                    ...
                    "time_savings": f"{pattern.average_time * pattern.optimization_potential:.1f} seconds per workflow",
                    "implementation": pattern.suggested_improvements[0] if pattern.suggested_improvements else "Streamline process",
                    "impact": "Medium"
                })

        return optimizations

    async def _calculate_efficiency(self, actions: List[UserAction]) -> Dict[str, float]:
        ...
        efficiency_score = (success_rate * 0.7 + min(1.0, 5.0 / avg_duration) * 0.3) * 100

        # Estimate time savings potential
        time_savings = max(0, avg_duration - 2.0) * len(actions)  # Assume 2s optimal time
    ...
    async def _get_recommendations(self, patterns: List[WorkflowPattern], efficiency: Dict) -> List[str]:
    ...
    async def get_workflow_insights(self, user_id: str, time_range: str = "7d") -> Dict[str, Any]:
        ...
        try:
            ...
            if not filtered_actions:
                return {
                    ...
                    "recommendations": ["Start using the platform to get personalized insights!"]
                }
            ...
            insights = {
                "summary": {
                    ...
                    "average_session_length": self._calculate_avg_session_length(filtered_actions)
                },
                "productivity_score": await self._calculate_productivity_score(filtered_actions),
                "top_patterns": await self._get_top_patterns(user_id),
                "efficiency_trends": await self._get_efficiency_trends(filtered_actions),
                "suggested_automations": await self._suggest_automations(filtered_actions)
            }
    ...
    def _get_most_active_day(self, actions: List[UserAction]) -> str:
        ...
        return max(day_counts.items(), key=lambda x: x[1])[0] if day_counts else "No data"
    ...
    def _calculate_avg_session_length(self, actions: List[UserAction]) -> float:
        ...
        for i in range(1, len(actions)):
            time_gap = (actions[i].timestamp - actions[i-1].timestamp).total_seconds() / 60
            if time_gap <= 30:  # Same session
                current_session.append(actions[i])
            else:  # New session
                if len(current_session) > 1:
                    session_length = (current_session[-1].timestamp - current_session[0].timestamp).total_seconds() / 60
    ...
    async def _calculate_productivity_score(self, actions: List[UserAction]) -> int:
        ...
        avg_speed = 1 / (statistics.mean([a.duration for a in actions if a.duration > 0]) + 1)
        action_variety = len(set(a.action_type for a in actions)) / 10  # Normalize to max 10 action types
    ...
    async def _get_top_patterns(self, user_id: str) -> List[Dict]:
        ...
        patterns = await self._detect_patterns(user_id)
        return sorted(patterns, key=lambda p: p.frequency * p.optimization_potential, reverse=True)[:3]
    ...
    async def _get_efficiency_trends(self, actions: List[UserAction]) -> List[Dict]:
        ...
        for day, durations in daily_efficiency.items():
            ...
            trends.append({
                "date": day.isoformat(),
                "efficiency": round(max(0, (5.0 - avg_duration) / 5.0 * 100), 1),  # 5s = 0% efficiency, 0s = 100%
    ...
    async def _suggest_automations(self, actions: List[UserAction]) -> List[str]:
        ...
        if action_counts.get("execute_request", 0) > 20:
            suggestions.append("🤖 Set up automated API health checks for your frequently tested endpoints")

        if action_counts.get("create_request", 0) > 15:
            suggestions.append("📋 Create request templates for your most common API patterns")

        if action_counts.get("switch_environment", 0) > 10:
            suggestions.append("🔄 Set up environment-specific test suites to reduce manual switching")
