...
class AIIntelligenceAgent:
    ...
    async def analyze(self, apis: List, spec: Dict) -> Dict:
        ...
        print(
            f"✅ AI analysis complete - Security Score: {result['security_score']}/100"
        )
        return result
    ...
    async def suggest_optimizations(self, api_spec: Dict) -> List[str]:
        ...
        for path, methods in api_spec.get("paths", {}).items():
            ...
            if "get" in methods and not any(
                p["name"] in ["limit", "offset", "page"]
                for p in methods["get"].get("parameters", [])
            ):
                optimizations.append(
                    f"Add pagination to GET {path} to improve performance"
                )
            ...
            if "post" in methods and "/bulk" not in path:
                optimizations.append(
                    f"Consider adding bulk operation endpoint for {path}"
                )
    ...
    async def check_compliance(self, api_spec: Dict) -> Dict:
        ...
        compliance = {
            "gdpr": {"compliant": False, "issues": [], "recommendations": []},
            "hipaa": {"compliant": False, "issues": [], "recommendations": []},
            "pci_dss": {"compliant": False, "issues": [], "recommendations": []},
            "sox": {"compliant": False, "issues": [], "recommendations": []},
        }

        # GDPR compliance checks
        has_delete_endpoints = any(
            "delete" in methods for path, methods in api_spec.get("paths", {}).items()
        )
        has_user_data = any(
            "user" in path.lower() or "profile" in path.lower()
            for path in api_spec.get("paths", {}).keys()
        )

        if has_user_data:
            if not has_delete_endpoints:
                compliance["gdpr"]["issues"].append(
                    "No DELETE endpoints for user data (Right to Erasure)"
                )
            compliance["gdpr"]["recommendations"].append(
                "Add data export endpoint for data portability"
            )
            compliance["gdpr"]["recommendations"].append(
        ...
        if "patient" in spec_str or "health" in spec_str:
            compliance["hipaa"]["issues"].append(
                "Ensure all health data endpoints use encryption"
            )
            compliance["hipaa"]["recommendations"].append(
        ...
        if "payment" in spec_str or "card" in spec_str:
            ...
            compliance["pci_dss"]["recommendations"].append(
    ...
    async def calculate_api_value(
        self, api_spec: Dict, test_results: Dict = None
    ) -> Dict:
        """
        ...
        hours_saved_testing = (
            num_endpoints * 5
        )  # 5 hours per endpoint for comprehensive tests

        total_hours_saved = (
            hours_saved_discovery + hours_saved_documentation + hours_saved_testing
        )
        ...
        try:
            ...
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert API architect and security analyst.",
                    },
