...
try:
    ...
    print(
        "⚠️ Warning: llm_decision_engine not available, using fallback implementation"
    )
...
class AutonomousSecurityAgent:
    ...
    async def autonomous_security_analysis(
        ...
        """
        self.logger.info(
            f"Starting autonomous security analysis for {endpoint_data.get('path', 'unknown')}"
        )
        ...
        self.logger.info(
            f"LLM created analysis plan with {len(analysis_plan.actions)} actions"
        )
        ...
        if isinstance(analysis_results, list):
            ...
            total_issues = (
                len(analysis_results.get("security_issues", []))
        ...
        return {
            ...
            "analysis_plan_summary": self.llm_engine.get_decision_summary(
                analysis_plan
            ),
            "action_plan_summary": self.llm_engine.get_decision_summary(action_plan),
            "total_issues_found": total_issues,
            "total_issues_fixed": len(
                [
                    r
            ...
            "execution_time": sum(
                r.get("duration", 0) for r in action_results if isinstance(r, dict)
            ),
            "security_issues": security_issues,  # Standardized format
            "recommendations": self._generate_recommendations(
                analysis_results, action_results
            ),
        }

    async def execute_approved_plan(
        self, plan_id: str, context: DecisionContext
    ) -> Dict[str, Any]:
        """
        ...
        plan = next(
            (p for p in self.llm_engine.decision_history if p.plan_id == plan_id), None
        )
    ...
    async def _execute_plan(
        self, plan: DecisionPlan, context: DecisionContext
    ) -> List[Dict[str, Any]]:
        """
        ...
        for action in plan.actions:
            try:
                ...
                if action.depends_on:
                    ...
                    dependency_results = [
                        r for r in results if r.get("action_id") in action.depends_on
                    ]
                    if not all(
                        r.get("status") == "success" for r in dependency_results
                    ):
                        results.append(
                            {
    ...
    async def _execute_single_action(
        ...
        if action.tool_name not in self.security_tools:
            return {"status": "error", "error": f"Unknown tool: {action.tool_name}"}

        tool_function = self.security_tools[action.tool_name]

        try:
            result = await tool_function(action.parameters, context)
            return {"status": "success", "tool": action.tool_name, "result": result}
        except Exception as e:
            return {"status": "error", "tool": action.tool_name, "error": str(e)}

    # Security tool implementations
    async def _run_vulnerability_scan(
        self, params: Dict[str, Any], context: DecisionContext
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            context.business_context.lower() if context.business_context else ""
            ...
            vulnerabilities.extend(
                self._detect_owasp_vulnerabilities(endpoint, industry)
            )
            ...
            if industry == "fintech":
                ...
                vulnerabilities.extend(
                    self._detect_healthcare_vulnerabilities(endpoint)
                )
            ...
            vulnerabilities.extend(
                self._detect_compliance_violations(endpoint, context)
            )

            # 5. General security infrastructure checks
            vulnerabilities.extend(
                self._detect_infrastructure_vulnerabilities(endpoint, context)
            )
    ...
    def _detect_industry_from_context(self, context: DecisionContext) -> str:
        ...
        if any(
            ...
            term in business_context + endpoint_path
            for term in ["iot", "device", "sensor", "machine", "management"]
        ):
            return "iot"
        else:
            return "general"

    def _detect_owasp_vulnerabilities(
        self, endpoint: Dict, industry: str
    ) -> List[Dict]:
    ...
    def _detect_compliance_violations(
        self, endpoint: Dict, context: DecisionContext
    ) -> List[Dict]:
    ...
    def _detect_infrastructure_vulnerabilities(
        self, endpoint: Dict, context: DecisionContext
    ) -> List[Dict]:
        """Detect general infrastructure and operational security vulnerabilities"""
        vulns = []
        str(endpoint.get("path", "") or "").lower()
    ...
    async def _analyze_auth_mechanism(
        self, params: Dict[str, Any], context: DecisionContext
    ) -> Dict[str, Any]:
        ...
        for scheme in security_schemes:
            ...
            if auth_type == "basic":
                ...
                auth_analysis["recommendations"].append(
                    "Use OAuth 2.0 or JWT tokens instead"
                )
            elif auth_type == "apiKey" and scheme.get("in") == "query":
                auth_analysis["weaknesses"].append(
                    "API key in query parameters is logged"
                )
    ...
    async def _check_data_exposure(
        ...
        return {"exposure_risks_found": len(exposure_risks), "risks": exposure_risks}
    ...
    async def _audit_session_management(
        self, params: Dict[str, Any], context: DecisionContext
    ) -> Dict[str, Any]:
    ...
    async def _analyze_cors_policy(
        self, params: Dict[str, Any], context: DecisionContext
    ) -> Dict[str, Any]:
    ...
    async def _scan_security_headers(
        self, params: Dict[str, Any], context: DecisionContext
    ) -> Dict[str, Any]:
    ...
    async def _auto_fix_security_headers(
        self, params: Dict[str, Any], context: DecisionContext
    ) -> Dict[str, Any]:
    ...
    async def _auto_update_env_vars(
        self, params: Dict[str, Any], context: DecisionContext
    ) -> Dict[str, Any]:
    ...
    def _generate_recommendations(
        self, analysis_results: List[Dict], action_results: List[Dict]
    ) -> List[str]:
        ...
        if total_vulnerabilities > 5:
            recommendations.append(
                "Consider implementing a comprehensive security review process"
            )
        ...
        if successful_fixes > 0:
            recommendations.append(
                f"Successfully automated {successful_fixes} security improvements"
            )

        recommendations.append(
            "Enable continuous security monitoring for ongoing protection"
        )
    ...
    def get_execution_analytics(self, user_id: str) -> Dict[str, Any]:
        ...
        user_executions = [
            e for e in self.execution_history if e.get("user_id") == user_id
        ]
        ...
        avg_duration = (
            sum(e.get("duration", 0) for e in user_executions) / total_executions
        )
