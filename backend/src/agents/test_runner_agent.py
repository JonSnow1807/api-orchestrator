...
class Assertion:
    ...
    def __init__(self, type: AssertionType, expected: Any, operator: str = "equals", description: str = ""):
        self.type = type
        self.expected = expected
        self.operator = operator  # equals, not_equals, greater_than, less_than, contains, regex
    ...
    def evaluate(self, response: httpx.Response, response_time_ms: float) -> bool:
        ...
        try:
            if self.type == AssertionType.STATUS_CODE:
                ...
                if not HAS_JSONPATH:
                    ...
                    try:
                        ...
                        self.passed = False
                        self.error_message = "Response is not valid JSON or path not found"
                else:
                    ...
                    try:
                        ...
                        if matches:
                            ...
                            self.actual = None
                            self.passed = False
                            self.error_message = f"JSONPath '{self.expected.get('path')}' not found"
            ...
            if not self.passed and not self.error_message:
                self.error_message = f"Expected {self.operator} {self.expected}, got {self.actual}"
...
class TestCase:
    ...
    async def execute(self, environment: Dict[str, str] = None, timeout: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            headers = self._replace_variables_dict(self.request.get('headers', {}), environment)
            params = self._replace_variables_dict(self.request.get('params', {}), environment)
            ...
            if self.teardown_script:
                exec(self.teardown_script, {"environment": environment, "response": self.response})
    ...
    def get_result(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "duration_ms": (self.end_time - self.start_time).total_seconds() * 1000 if self.end_time and self.start_time else 0,
            "assertions": []
        }
...
class TestSuite:
    ...
    async def run(self, environment: Dict[str, str] = None, parallel: bool = False) -> Dict[str, Any]:
...
class TestRunnerAgent:
    ...
    def create_assertion(self, type_str: str, expected: Any, operator: str = "equals", description: str = "") -> Assertion:
    ...
    def create_test_from_request(self, 
                                 name: str,
                                 url: str,
                                 method: str = "GET",
                                 headers: Dict = None,
                                 body: Any = None,
                                 assertions: List[Dict] = None) -> TestCase:
    ...
    async def run_single_test(self, test: TestCase, environment: Dict[str, str] = None) -> Dict[str, Any]:
        """Run a single test case"""
        result = await test.execute(environment)
        self.history.append(result)
        return result
    
    async def run_suite(self, suite: TestSuite, environment: Dict[str, str] = None, parallel: bool = False) -> Dict[str, Any]:
        """Run a test suite"""
    ...
    def create_default_assertions(self, expected_status: int = 200) -> List[Assertion]:
        ...
        return [
            Assertion(AssertionType.STATUS_CODE, expected_status, "equals", "Status code should be " + str(expected_status)),
            Assertion(AssertionType.RESPONSE_TIME, 2000, "less_than", "Response time should be under 2 seconds"),
            Assertion(AssertionType.IS_JSON, True, "equals", "Response should be valid JSON")
        ]
    ...
    def generate_postman_assertions(self, postman_tests: str) -> List[Assertion]:
        ...
        patterns = [
            ...
            (r'pm\.expect\(pm\.response\.text\(\)\)\.to\.include\(["\'](.+?)["\']\)', AssertionType.BODY_CONTAINS),
            (r'pm\.response\.to\.have\.header\(["\'](.+?)["\']\)', AssertionType.HEADER_EXISTS),
        ]
