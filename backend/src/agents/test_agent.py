...
from src.core.orchestrator import AgentMessage
...
'''
    ...
    def _generate_pytest_test_method(
        self,
        path: str,
        method: str,
        operation: Dict,
    ...
    def _generate_postman_collection(self, spec: Dict, options: Dict) -> List[Dict]:
        ...
        collection = {
            ...
            "variable": [
                {
                    "key": "baseUrl",
                    "value": spec.get("servers", [{"url": "http://localhost:8000"}])[0][
                        "url"
                    ],
                }
            ],
    ...
    def _create_postman_request(self, path: str, method: str, operation: Dict) -> Dict:
        ...
        if method.upper() in ["POST", "PUT", "PATCH"] and operation.get("requestBody"):
            request["request"]["body"] = {
                "mode": "raw",
                "raw": json.dumps(
                    self._generate_sample_data(operation["requestBody"]), indent=2
                ),
        ...
        for path, methods in paths.items():
            for method, operation in methods.items():
                if method.upper() in ["GET", "POST", "PUT", "DELETE"]:
                    ...
                    weight = (
                        10 if method.upper() == "GET" else 5
                    )  # GET requests more frequent
...
'''
                    ...
                    if path_params:
                        for param in path_params:
                            url = url.replace(
                                f"{{{param}}}", f"{{random.randint(1, 100)}}"
                            )
                        url = f'f"{url}"'
                    else:
                        url = f'"{url}"'
...
'''
        ...
        for path, methods in paths.items():
            for method, operation in methods.items():
                test_method = self._generate_unittest_method(
...
"""
    ...
    def _generate_unittest_method(
        self, path: str, method: str, operation: Dict, base_url: str
    ) -> str:
