...
class PluginMarketplace:
    ...
    async def publish_plugin(
        ...
        # Store package
        stored_path = self._store_package(package_path, plugin.manifest.id, plugin.manifest.version)
    ...
    async def install_plugin(
        ...
        if plugin.status != PluginStatus.PUBLISHED and plugin.status != PluginStatus.APPROVED:
            raise ValueError(f"Plugin {plugin_id} is not available for installation")
        
        # Check compatibility
        if not self._check_compatibility(plugin.manifest):
            raise ValueError(f"Plugin {plugin_id} is not compatible with current version")
    ...
    async def uninstall_plugin(
        ...
        self.installations[workspace_id] = [
            inst for inst in self.installations[workspace_id]
            if inst.plugin_id != plugin_id
        ]
    ...
    async def get_plugin_details(
        ...
        return {
            ...
            "compatibility": {
                "current_version_compatible": self._check_compatibility(plugin.manifest),
    ...
    async def _install_dependencies(self, manifest: PluginManifest, install_dir: Path):
        ...
        if manifest.runtime == "nodejs" and manifest.dependencies:
            ...
            # Run pip install
            subprocess.run(["pip", "install", "-r", "requirements.txt"], cwd=install_dir, check=True)
    ...
    async def _run_deactivation_hooks(self, manifest: PluginManifest, install_dir: Path):
