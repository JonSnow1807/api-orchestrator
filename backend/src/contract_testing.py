...
class ContractValidator:
    ...
    def validate_request(request_data: Dict, contract: APIContract) -> Dict[str, Any]:
        ...
        if contract.validation_rules:
            rule_results = ContractValidator._validate_custom_rules(
                request_data, 
                contract.validation_rules.get("request", [])
            )
    ...
    def validate_response(
        ...
        if contract.headers_schema:
            header_errors = ContractValidator._validate_headers(headers, contract.headers_schema)
        ...
        if contract.validation_rules:
            rule_results = ContractValidator._validate_custom_rules(
                response_data,
                contract.validation_rules.get("response", [])
            )
            errors.extend(rule_results["errors"])
            warnings.extend(rule_results["warnings"])
            ...
            if header_name in headers:
                ...
                if "pattern" in header_spec:
                    ...
                    if not re.match(header_spec["pattern"], header_value):
                        errors.append(f"Header {header_name} doesn't match pattern: {header_spec['pattern']}")
            ...
            if rule_type == "required_field":
                ...
                if value and format_pattern:
                    ...
                    if not re.match(format_pattern, str(value)):
                        errors.append(f"Field {field_path} doesn't match format: {format_pattern}")
...
class ContractTestRunner:
    ...
    async def run_contract_tests(
        ...
        for endpoint in contract.test_endpoints or []:
            ...
            # Validate request against contract
            request_validation = ContractValidator.validate_request(request_data, contract)
            ...
            try:
                async with httpx.AsyncClient() as client:
                    response = await client.request(
                        ...
                        json=request_data if endpoint["method"] in ["POST", "PUT", "PATCH"] else None,
                        headers=headers
                    )
                    ...
                    if response_validation["warnings"]:
                        results["summary"]["warnings"] += len(response_validation["warnings"])
        ...
        contract.last_test_status = "passed" if results["summary"]["failed"] == 0 else "failed"
