...
class Variable(Base):
    ...
    id = Column(
        String, primary_key=True, default=lambda: f"var_{secrets.token_urlsafe(16)}"
    )
...
class VariableHistory(Base):
    ...
    id = Column(
        String, primary_key=True, default=lambda: f"vh_{secrets.token_urlsafe(16)}"
    )
...
class EnhancedVariableManager:
    ...
    async def create_variable(
        ...
        **kwargs,
        ...
        variable = Variable(
            ...
            current_value=value,
            scope=scope.value,
            visibility=VariableVisibility.PRIVATE.value
            if scope == VariableScope.LOCAL
            else VariableVisibility.TEAM.value,
    ...
    async def get_variable_value(
        self, key: str, user_id: str, context: Optional[Dict] = None
    ) -> Any:
    ...
    async def update_variable(
        ...
        # Update variable
        old_value = variable.current_value
        variable.current_value = self._encrypt_if_sensitive(
            new_value, variable.is_sensitive
        )
        variable.version += 1
    ...
    async def share_variable(
        ...
        if "users" in share_with:
            variable.shared_with_users = list(
                set(variable.shared_with_users + share_with["users"])
            )

        if "teams" in share_with:
            variable.shared_with_teams = list(
                set(variable.shared_with_teams + share_with["teams"])
            )
        ...
        if expires_in_hours:
            ...
            variable.share_expires_at = datetime.utcnow() + timedelta(
        ...
        if unshare_from:
            ...
            if "users" in unshare_from:
                variable.shared_with_users = [
                    u
                    for u in variable.shared_with_users
            ...
            if "teams" in unshare_from:
                variable.shared_with_teams = [
                    t
                    for t in variable.shared_with_teams
    ...
    async def bulk_update_variables(
        self, updates: List[Dict[str, Any]], user_id: str
    ) -> List[Variable]:
    ...
    def _has_access(self, variable: Variable, user_id: str) -> bool:
        ...
        if user_id in variable.shared_with_users:
            # Check expiration
            if (
                variable.share_expires_at
                and variable.share_expires_at < datetime.utcnow()
            ):
                return False
            return True
    ...
    def _can_modify(self, variable: Variable, user_id: str) -> bool:
        ...
        return (
            self._is_owner(variable, user_id) or user_id in variable.shared_with_users
        )
