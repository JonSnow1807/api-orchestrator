...
class ScheduledMonitorManager:
    ...
    def _schedule_monitor(self, monitor: MonitorSchedule):
        ...
        if monitor.schedule_type == "cron":
            trigger = CronTrigger.from_crontab(
                monitor.cron_expression, timezone=monitor.timezone
            )
        elif monitor.schedule_type == "interval":
            trigger = IntervalTrigger(
                seconds=monitor.interval_seconds, timezone=monitor.timezone
            )
        else:
    ...
    async def _execute_monitor(self, monitor_id: int):
        ...
        for endpoint in monitor.endpoints:
            try:
                ...
                results.append(
                    {"endpoint": endpoint["url"], "is_healthy": False, "error": str(e)}
                )
        ...
        healthy_count = sum(
            1
            for r in results
            if (r.is_healthy if hasattr(r, "is_healthy") else r.get("is_healthy"))
        )
    ...
    def get_monitor_status(self, monitor_id: int) -> Dict[str, Any]:
        ...
        if monitor.recent_results:
            ...
            successful = sum(
                1
                for r in monitor.recent_results
        ...
        return {
            ...
            "recent_results": monitor.recent_results[-10:]
            if monitor.recent_results
            else [],
