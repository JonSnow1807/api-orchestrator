...
class JiraIntegration:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            async with self.session.get(
                f"{self.config.base_url}/rest/api/3/myself", headers=headers
            ) as response:
    ...
    async def create_issue(
        ...
        async with self.session.post(
            f"{self.config.base_url}/rest/api/3/issue", headers=headers, json=issue_data
        ) as response:
    ...
    async def search_issues(
        ...
        async with self.session.get(
            f"{self.config.base_url}/rest/api/3/search", headers=headers, params=params
        ) as response:
            if response.status == 200:
                data = await response.json()
...
class SlackIntegration:
    ...
    async def create_alert_message(
        ...
        attachments = [{"color": color, "fallback": f"{title}: {description}"}]
        ...
        sections = [
            {
                ...
                "facts": [
                    {"name": key, "value": str(value)} for key, value in details.items()
                ],
                "markdown": True,
            }
        ]
...
class GitHubIntegration:
    ...
    async def create_pull_request_comment(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                url, headers=headers, json=comment_data
            ) as response:
                return response.status == 201
    ...
    async def update_commit_status(
        ...
        status_data = {"state": state, "context": context}
...
class DataDogIntegration:
    ...
    async def send_metric(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.base_url}/series", headers=headers, json=series
            ) as response:
    ...
    async def create_event(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.base_url}/events", headers=headers, json=event_data
            ) as response:
                return response.status == 202
...
class PagerDutyIntegration:
    ...
    async def trigger_incident(
        ...
        # Generate dedup key
        dedup_key = hashlib.sha256(f"{summary}{source}{component}".encode()).hexdigest()
        event_data["dedup_key"] = dedup_key

        async with aiohttp.ClientSession() as session:
            async with session.post(
                "https://events.pagerduty.com/v2/enqueue", json=event_data
            ) as response:
...
class IntegrationManager:
    ...
    async def add_integration(self, integration: Integration) -> Integration:
        """Add new integration"""
        ...
        try:
            if integration.type == IntegrationType.SLACK:
                await self._handle_slack_event(
                    handler, event_type, event_data, integration.settings
                )
            elif integration.type == IntegrationType.JIRA:
                await self._handle_jira_event(
                    handler, event_type, event_data, integration.settings
                )
            elif integration.type == IntegrationType.TEAMS:
                await self._handle_teams_event(
                    handler, event_type, event_data, integration.settings
                )
                ...
                await self._handle_datadog_event(
                    handler, event_type, event_data, integration.settings
                )
            elif integration.type == IntegrationType.PAGERDUTY:
                await self._handle_pagerduty_event(
                    handler, event_type, event_data, integration.settings
                )
        except Exception as e:
