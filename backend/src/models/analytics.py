...
class AnalyticsService:
    ...
    def calculate_roi(workspace_id: int, db) -> Dict[str, Any]:
        ...
        metrics = db.query(BusinessMetrics).filter(
            BusinessMetrics.workspace_id == workspace_id
        ).order_by(BusinessMetrics.period_end.desc()).first()
        
        if not metrics:
            return {"roi": 0, "savings": 0, "message": "No data available"}
        
        roi = ((metrics.total_cost_saved - metrics.platform_cost) / 
               metrics.platform_cost * 100) if metrics.platform_cost > 0 else 0
    ...
    def get_performance_trends(workspace_id: int, days: int, db) -> List[Dict]:
        ...
        ).order_by(PerformanceSnapshot.period_start).all()
    ...
    def generate_insights(workspace_id: int, db) -> List[str]:
        ...
        # Get recent metrics
        recent_metrics = db.query(APIMetrics).filter(
        ...
        if recent_metrics:
            ...
            if error_rate > 5:
                insights.append(f"⚠️ High error rate detected ({error_rate:.1f}%). Consider reviewing error logs.")
            
            avg_response = sum(m.response_time_ms for m in recent_metrics if m.response_time_ms) / len(recent_metrics)
            if avg_response > 1000:
                insights.append(f"🐌 Slow API responses averaging {avg_response:.0f}ms. Consider optimization.")
            
            # Cost analysis
            total_cost = sum(m.estimated_cost for m in recent_metrics if m.estimated_cost)
            if total_cost > 100:
                insights.append(f"💰 High API costs detected (${total_cost:.2f} this week). Review usage patterns.")
