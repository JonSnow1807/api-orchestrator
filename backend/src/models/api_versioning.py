...
class APIVersion(Base):
    ...
    changelog_entries = relationship(
        "APIChangelog", back_populates="version", cascade="all, delete-orphan"
    )
    ...
    def compare_with(self, other_version: "APIVersion") -> Dict[str, Any]:
        ...
        if self.spec_format == "openapi" and other_version.spec_format == "openapi":
            comparison = self._compare_openapi_specs(
                self.spec_content, other_version.spec_content, comparison
            )
    ...
    def _compare_openapi_specs(
        self, spec_a: Dict, spec_b: Dict, comparison: Dict
    ) -> Dict:
        ...
        for path in paths_a:
            if path in paths_b:
                ...
                if added_methods:
                    comparison["additions"].append(
                        {"type": "method", "path": path, "methods": list(added_methods)}
                    )
                ...
                for method in methods_a & methods_b:
                    ...
                    required_params_b = {
                        p["name"] for p in params_b if p.get("required", False)
                    }
                    required_params_a = {
                        p["name"] for p in params_a if p.get("required", False)
                    }
...
class APIChangelog(Base):
    ...
    change_type = Column(
        String(50)
    )  # "feature", "bugfix", "breaking", "deprecation", "security"
    change_category = Column(
        String(50)
    )  # "endpoint", "schema", "authentication", "parameter"
...
class APIVersionNotification(Base):
    ...
    notification_type = Column(
        String(50)
    )  # "new_version", "deprecation", "sunset", "breaking_change"
...
class APIVersioningService:
    ...
    def create_version(
        ...
        current_version = (
            db.query(APIVersion)
            .filter(APIVersion.api_id == api_id, APIVersion.is_current == True)
            .first()
        )
            ...
            timeline.append(
                {
                    ...
                    "created_by": version.creator.username
                    if version.creator
                    else "Unknown",
    ...
    def rollback_version(
        db, api_id: int, target_version_id: int, user_id: int
    ) -> APIVersion:
        """Rollback to a previous API version"""
        target_version = (
            db.query(APIVersion)
            .filter(APIVersion.id == target_version_id, APIVersion.api_id == api_id)
            .first()
        )
