...
class Workspace(Base):
    ...
    webhooks = relationship(
        "WorkspaceWebhook", back_populates="workspace", cascade="all, delete-orphan"
    )
    ai_keys = relationship(
        "AIKey", back_populates="workspace", cascade="all, delete-orphan"
    )
...
class WorkspaceWebhook(Base):
    ...
    creator = relationship(
        "User", foreign_keys=[created_by], backref="created_webhooks"
    )
...
def update_existing_models():
    ...
    if not hasattr(User, "last_active_workspace_id"):
        User.last_active_workspace_id = Column(
            Integer, ForeignKey("workspaces.id"), nullable=True
        )
