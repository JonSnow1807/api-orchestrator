            ...
            if isinstance(field, str):
                ...
                for key, subfields in field.items():
                    if isinstance(subfields, list):
                        sub_selection = GraphQLQueryBuilder._build_fields(
                            subfields, indent + 1
                        )
...
class GraphQLIntrospector:
    ...
    async def introspect(
        endpoint_url: str, headers: Optional[Dict[str, str]] = None
    ) -> Dict:
    ...
    def extract_operations(schema: Dict) -> Dict[str, List[Dict]]:
        ...
        for type_def in schema.get("types", []):
            if type_def.get("name") == query_type:
                ...
                operations["subscriptions"] = GraphQLIntrospector._extract_fields(
                    type_def
                )
...
class GraphQLExecutor:
    ...
    async def execute(
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                endpoint_url, json=payload, headers=headers or {}, timeout=30.0
            )
    ...
    def validate_response(
        response: Dict, expected_schema: Optional[Dict] = None
    ) -> List[str]:
...
class GraphQLTestGenerator:
    ...
    @staticmethod
    def generate_tests(
        schema: Dict, operations: Dict[str, List[Dict]], test_framework: str = "pytest"
    ) -> List[Dict]:
        ...
        if test_framework == "pytest":
            tests.extend(
                GraphQLTestGenerator._generate_pytest_tests(schema, operations)
            )
