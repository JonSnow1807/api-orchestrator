...
class OfflineManager:
    ...
    async def save_collection(
        self, collection: OfflineCollection, format: StorageFormat = StorageFormat.BRU
    ) -> str:
    ...
    async def _load_bru_collection(self, path: Path) -> OfflineCollection:
        ...
        collection = OfflineCollection(
            id=hashlib.md5(str(path).encode()).hexdigest(), name=metadata["name"]
        )
    ...
    async def _load_http_collection(self, path: Path) -> OfflineCollection:
        ...
        collection = OfflineCollection(
            id=hashlib.md5(str(path).encode()).hexdigest(), name=path.name
        )
        ...
        for http_file in path.glob("**/*.http"):
            ...
            for request in requests:
                request.folder = (
                    http_file.parent.name if http_file.parent != path else None
                )
                collection.requests.append(request)
        ...
        for request in collection.requests:
            item = {
                ...
                "request": {
                    ...
                    "url": request.url,
                    "header": [
                        {"key": k, "value": v} for k, v in request.headers.items()
                    ],
                },
            }

            if request.body:
                item["request"]["body"] = {"mode": "raw", "raw": request.body}

            postman_collection["item"].append(item)

        # Convert variables
        for key, value in collection.variables.items():
            postman_collection["variable"].append({"key": key, "value": value})
    ...
    def import_from_postman(self, postman_data: Dict[str, Any]) -> OfflineCollection:
        ...
        collection = OfflineCollection(
            id=info.get(
                "_postman_id",
                hashlib.md5(json.dumps(postman_data).encode()).hexdigest(),
            ),
