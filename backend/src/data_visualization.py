    ...
    def _analyze_list(self, data: List) -> Dict[str, Any]:
        ...
        if all(isinstance(item, dict) for item in data):
            ...
            if data[0]:
                field_types = {}
                for key in all_keys:
                    sample_value = next((item.get(key) for item in data if key in item), None)
                ...
                has_categorical = any(ft == 'categorical' for ft in field_types.values())
                ...
                if has_time and has_numerical:
                    ...
                    analysis['insights'].append("Time series data detected - line or area chart recommended")
                
                if has_categorical and has_numerical:
                    analysis['recommended_viz'].extend([
                        VisualizationType.BAR.value,
                        VisualizationType.PIE.value
                    ])
                    analysis['insights'].append("Categorical data with values - bar or pie chart recommended")
                
                if has_numerical and len(all_keys) >= 2:
                    analysis['recommended_viz'].append(VisualizationType.SCATTER.value)
                    analysis['insights'].append("Multiple numerical fields - scatter plot available")
    ...
    def process_natural_language_query(self, data: Any, query: str) -> Dict[str, Any]:
        ...
        # Average operations
            ...
            if field:
                ...
                if values:
                    ...
                    result['data'] = {'average': avg, 'field': field, 'count': len(values)}
                    result['success'] = True
        ...
        # Sort operations
            ...
            if field:
                ...
                result['success'] = True
                result['operation'] = f"Sorted by {field} {'descending' if reverse else 'ascending'}"
        ...
        if isinstance(data, list):
            ...
            if all(isinstance(item, dict) for item in data):
                ...
                for field in numerical_fields:
                    values = [item.get(field, 0) for item in data if isinstance(item.get(field), (int, float))]
                    if values:
                        insights.append(f"{field}: min={min(values):.2f}, max={max(values):.2f}, avg={statistics.mean(values):.2f}")
            ...
            numerical_count = sum(1 for v in data.values() if isinstance(v, (int, float)))
            if numerical_count:
