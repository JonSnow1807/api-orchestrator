...
class ServiceVirtualizer:
    ...
    async def start_service(self, service_id: str):
        ...
        print(f"Virtual service '{service.name}' started at http://localhost:{service.port}{service.base_path}")
    ...
    def _extract_resource_id(self, path: str, pattern: str) -> Optional[str]:
        ...
        regex_pattern = pattern.replace("{id}", r"([^/]+)").replace("{", r"\{").replace("}", r"\}")
    ...
    async def _record_and_forward(self, rule: MockRule, request, service: VirtualService):
    ...
    def _replay_response(self, rule: MockRule, request, service: VirtualService) -> Dict[str, Any]:
...
# Export for use
__all__ = ['ServiceVirtualizer', 'VirtualService', 'MockRule', 'MockBehavior', 'ChaosEngine']