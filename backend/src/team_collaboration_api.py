...
async def get_workspaces(
    ...
    for workspace in workspaces:
        ...
        workspace_data.user_role = checker.get_user_workspace_role(
...
async def update_member(
    ...
    activity = WorkspaceActivityLog(
        ...
        details={
            "target_user": user_id,
            "changes": member_update.dict(exclude_none=True),
        },
    )
    db.add(activity)
    db.commit()
...
async def get_collections(
    ...
    # Get collections
    collections = (
        db.query(Collection).filter(Collection.workspace_id == workspace_id).all()
    )
    ...
    for collection in collections:
        if collection.can_user_access(current_user.id, member):
            accessible_collections.append(
                collection.to_dict(include_requests=include_requests)
            )
...
async def get_activity_log(
    ...
    activities = (
        ...
        .filter(WorkspaceActivityLog.workspace_id == workspace_id)
...
async def send_invitation_email(
    email: str, workspace_name: str, invited_by: str, token: str, role: str
):
...
async def get_my_permissions(
    ...
    sharing_manager = SharingManager(db)
    shared_resources = sharing_manager.get_user_shared_resources(
        current_user.id, workspace_id
    )

    return {"workspace_permissions": permissions, "shared_resources": shared_resources}
