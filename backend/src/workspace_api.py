...
from src.models.workspace import (
    ...
    WorkspaceWebhook, ResourcePermission, WorkspaceRole,
    ResourceType, workspace_members
)
from src.email_service import send_invitation_email, send_notification_email
...
async def create_workspace(
    ...
    existing_workspaces = db.query(Workspace).filter(
    ...
    # Log activity
    log_activity(
        db, workspace.id, current_user.id, "workspace_created",
...
async def get_my_workspaces(
    ...
    for workspace in workspaces:
        member_count = db.query(func.count(workspace_members.c.user_id)).filter(
...
async def update_workspace(
    ...
    # Log activity
    log_activity(
        db, workspace.id, current_user.id, "workspace_updated",
...
async def invite_member(
    ...
    # Log activity
    log_activity(
        db, workspace_id, current_user.id, "member_invited",
...
async def accept_invitation(
    ...
    # Log activity
    log_activity(
        db, workspace_id, current_user.id, "invitation_accepted",
...
async def update_member_role(
    ...
    # Log activity
    log_activity(
        db, workspace_id, current_user.id, "member_role_updated",
...
async def remove_member(
    ...
    # Log activity
    log_activity(
        db, workspace_id, current_user.id, "member_removed",
...
async def get_workspace_activity(
    ...
    activities = db.query(WorkspaceActivity).filter(
        ...
        WorkspaceActivity.created_at.desc()
...
async def get_workspace_analytics(
    ...
    project_count = db.query(func.count()).select_from(Project).filter(
        Project.workspace_id == workspace_id
    ).scalar() if hasattr(db, 'Project') else 0
    
    # Activity stats for last 30 days
    thirty_days_ago = datetime.utcnow() - timedelta(days=30)
    recent_activities = db.query(
        WorkspaceActivity.action,
        func.count(WorkspaceActivity.id).label('count')
    ).filter(
        and_(
            WorkspaceActivity.workspace_id == workspace_id,
            WorkspaceActivity.created_at >= thirty_days_ago
        )
    ).group_by(WorkspaceActivity.action).all()
    ...
    return {
        ...
        "recent_activity": {
            activity.action: activity.count 
