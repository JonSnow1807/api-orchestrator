...
class APIInsights:
    ...
    async def ingest_metric(self, metric: APIMetric):
        ...
        alpha = 0.1
        health.avg_response_time = alpha * metric.response_time_ms + (1 - alpha) * health.avg_response_time
        ...
        for error_key, errors in error_groups.items():
            ...
            if pattern_id not in self.failure_patterns:
                ...
                insight = Insight(
                    ...
                    severity=InsightSeverity.HIGH if int(status_code) >= 500 else InsightSeverity.MEDIUM,
                    ...
                    recommendations=self._get_error_recommendations(pattern.pattern_type)
                )
                self.insights.append(insight)
    ...
    async def _analyze_version_migration(self):
        ...
        for version, data in version_counts.items():
            usage = VersionUsage(
                version=version,
                usage_percentage=(data["requests"] / total_requests * 100) if total_requests > 0 else 0,
                total_requests=data["requests"],
                unique_consumers=len(data["consumers"]),
                deprecated="deprecated" in version.lower() or version < "v2",  # Example logic
                sunset_date=datetime.now() + timedelta(days=90) if "v1" in version else None,
                migration_status={}
            )
    ...
    async def _check_sla_violations(self):
        ...
        for endpoint_key, health in self.endpoint_health.items():
            ...
            if health.avg_response_time > self.sla_thresholds["response_time_ms"]:
                violations.append(f"Response time {health.avg_response_time:.0f}ms exceeds SLA")
    ...
    async def _detect_anomalies(self):
        ...
        baseline_metrics = [m for m in self.metrics_buffer if baseline_cutoff < m.timestamp < current_cutoff]
        current_metrics = [m for m in self.metrics_buffer if m.timestamp > current_cutoff]
        
        if len(baseline_metrics) < 100 or len(current_metrics) < 10:
            return
            
        # Compare traffic volume
        baseline_volume = len(baseline_metrics) / (self.baseline_window.total_seconds() / 3600)
        ...
        if abs(current_volume - baseline_volume) > baseline_volume * 0.5:
            ...
            insight = Insight(
                ...
                recommendations=[
                    "Check for DDoS attacks" if anomaly_type == "spike" else "Check service availability",
                    "Review recent deployments",
                    "Monitor resource utilization",
                    "Prepare to scale" if anomaly_type == "spike" else "Investigate root cause"
                ]
            )
            self.insights.append(insight)
    ...
    def _get_error_recommendations(self, pattern_type: str) -> List[str]:
        ...
        return recommendations.get(pattern_type, ["Investigate error logs", "Review recent changes"])
    ...
    def _detect_usage_pattern(self, metrics: List[APIMetric]) -> str:
        ...
        time_diffs = [
            (sorted_metrics[i+1].timestamp - sorted_metrics[i].timestamp).total_seconds()
    ...
    async def get_insights_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_severity": {
                "critical": len([i for i in active_insights if i.severity == InsightSeverity.CRITICAL]),
                "high": len([i for i in active_insights if i.severity == InsightSeverity.HIGH]),
                "medium": len([i for i in active_insights if i.severity == InsightSeverity.MEDIUM]),
                "low": len([i for i in active_insights if i.severity == InsightSeverity.LOW]),
                "info": len([i for i in active_insights if i.severity == InsightSeverity.INFO])
            },
            "by_type": {
                insight_type.value: len([i for i in active_insights if i.type == insight_type])
                for insight_type in InsightType
            },
            "recent_insights": [i.dict() for i in active_insights[:10]],
            "endpoint_health": {
                "healthy": len([h for h in self.endpoint_health.values() if h.health_score > 80]),
                "warning": len([h for h in self.endpoint_health.values() if 60 < h.health_score <= 80]),
                "critical": len([h for h in self.endpoint_health.values() if h.health_score <= 60])
            },
            "version_usage": {
                v.version: {
