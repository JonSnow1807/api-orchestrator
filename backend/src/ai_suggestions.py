...
try:
    ...
    AI_AVAILABLE = False
    print("Warning: AI packages not available. Inline suggestions will use fallback mode.")
...
class InlineAISuggestions:
    ...
    async def get_header_suggestions(
        ...
        if not AI_AVAILABLE or not self.llm:
            return self._get_fallback_header_suggestions(method, endpoint, existing_headers)
...
Format: JSON array of {{"issue": "description", "severity": "critical|high|medium|low", "fix": "solution", "reference": "OWASP/standard"}}"""
    ...
    def _get_fallback_header_suggestions(
        ...
        suggestions = [
            {"header": "Content-Type", "value": "application/json", "required": True, "description": "Specify content type"},
            {"header": "Accept", "value": "application/json", "required": False, "description": "Specify accepted response type"},
            {"header": "Authorization", "value": "Bearer <token>", "required": True, "description": "Authentication token"},
            {"header": "X-Request-ID", "value": "<unique-id>", "required": False, "description": "Track requests"},
        ]
        
        # Filter out existing headers
        if existing_headers:
            suggestions = [s for s in suggestions if s["header"] not in existing_headers]
    ...
    def _get_fallback_payload_suggestions(self, endpoint: str, method: str) -> AISuggestion:
    ...
    def _get_fallback_test_suggestions(self, status_code: int, response_data: Dict[str, Any]) -> AISuggestion:
        """Fallback test suggestions"""
        
        suggestions = [
            {"assertion": "Status code check", "path": "status", "expected": status_code, "type": "equals"},
            {"assertion": "Response not empty", "path": "body", "expected": None, "type": "exists"},
            {"assertion": "Has data field", "path": "data", "expected": None, "type": "exists"},
        ]
    ...
    def _get_fallback_optimization_suggestions(self, response_time_ms: float) -> AISuggestion:
    ...
    def _get_fallback_security_suggestions(self, headers: Dict[str, str]) -> AISuggestion:
