...
class WebSocketTester:
                    ...
                    if wait_for_response:
                        try:
                            ...
                            results["errors"].append(
                                f"Response timeout for message: {message[:100]}"
                            )
    ...
    def generate_test_messages(endpoint_type: str) -> List[Dict]:
        ...
        if endpoint_type == "chat":
            ...
            return [{"message": "test", "wait_for_response": True}]
...
<soap:Envelope xmlns:soap="{namespace}">
    <soap:Body>
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(url, content=envelope, headers=soap_headers)
    ...
    async def call_grpc_method(
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                full_url, content=request_body, headers=headers
            )
...
class SSETester:
    ...
    async def connect_and_listen(
        ...
        results = {"connection": None, "events": [], "errors": [], "duration_ms": 0}
