...
from src.team_collaboration_models import (
    Workspace, WorkspaceMember, ActiveSession, CollaborationEvent,
...
class CollaborationManager:
    ...
    async def connect(self, websocket: WebSocket, user: User, workspace_id: int, db: Session):
        ...
        await self.broadcast_presence_update(workspace_id, user.id, exclude_user=user.id)
    ...
    async def disconnect(self, websocket: WebSocket, workspace_id: int, user_id: int, db: Session):
        ...
        try:
            ...
            if (workspace_id in self.active_connections and 
                ...
                if not self.active_connections[workspace_id][user_id]:
                    ...
                    if (workspace_id in self.user_presence and 
                        user_id in self.user_presence[workspace_id]):
                        self.user_presence[workspace_id][user_id]['status'] = 'away'
                        self.user_presence[workspace_id][user_id]['last_seen'] = datetime.utcnow().isoformat()
                    
                    # Release any resource locks
                    await self.release_user_locks(workspace_id, user_id)
                    
                    # Notify other users
                    await self.broadcast_presence_update(workspace_id, user_id, exclude_user=user_id)
            
            # Clean up sessions
            sessions_to_remove = []
            for token, session_data in self.active_sessions.items():
                if (session_data['workspace_id'] == workspace_id and 
    ...
    async def send_initial_data(self, websocket: WebSocket, workspace_id: int, user_id: int, db: Session):
        ...
        try:
            ...
            # Get recent activity
            recent_activity = db.query(WorkspaceActivityLog).filter(
                WorkspaceActivityLog.workspace_id == workspace_id,
                WorkspaceActivityLog.created_at > datetime.utcnow() - timedelta(hours=1)
            ).order_by(WorkspaceActivityLog.created_at.desc()).limit(10).all()
            
            # Send initial data
            await self.send_personal_message(websocket, {
                'type': 'initial_data',
                'data': {
                    'presence': {uid: data for uid, data in presence.items() if uid != user_id},
                    'recent_activity': [activity.to_dict() for activity in recent_activity],
                    'active_collaborators': len(presence),
                    'your_session': self.get_user_session_info(workspace_id, user_id)
                }
            })
    ...
    async def handle_message(self, websocket: WebSocket, message: str, workspace_id: int, user_id: int, db: Session):
        ...
        try:
            ...
            if message_type == 'presence_update':
                ...
                await self.handle_resource_lock(websocket, workspace_id, user_id, payload)
                
            elif message_type == 'resource_unlock':
                await self.handle_resource_unlock(websocket, workspace_id, user_id, payload)
                
            elif message_type == 'collaboration_event':
                await self.handle_collaboration_event(workspace_id, user_id, payload, db)
    ...
    async def handle_presence_update(self, workspace_id: int, user_id: int, payload: dict, db: Session):
    ...
    async def handle_cursor_update(self, workspace_id: int, user_id: int, payload: dict):
        ...
        await self.broadcast_to_resource(workspace_id, resource_id, {
            'type': 'cursor_update',
            'data': {
                'user_id': user_id,
                'resource_id': resource_id,
                'cursor': self.collaboration_cursors[workspace_id][resource_id][user_id]
            }
        }, exclude_user=user_id)

    async def handle_resource_lock(self, websocket: WebSocket, workspace_id: int, user_id: int, payload: dict):
    ...
    async def handle_resource_unlock(self, websocket: WebSocket, workspace_id: int, user_id: int, payload: dict):
        ...
        if (workspace_id in self.resource_locks and 
            self.resource_locks[workspace_id].get(resource_id) == user_id):
    ...
    async def handle_collaboration_event(self, workspace_id: int, user_id: int, payload: dict, db: Session):
        ...
        await self.broadcast_to_resource(workspace_id, str(resource_id), {
    ...
    async def handle_typing_indicator(self, workspace_id: int, user_id: int, payload: dict):
    ...
    async def broadcast_to_workspace(self, workspace_id: int, message: dict, exclude_user: int = None):
        ...
        for user_id, connections in self.active_connections[workspace_id].items():
            ...
            for websocket in connections[:]:  # Create copy to avoid modification during iteration
                try:
    ...
    async def broadcast_to_resource(self, workspace_id: int, resource_id: str, message: dict, exclude_user: int = None):
        ...
        for user_id, presence in self.user_presence[workspace_id].items():
            if (presence.get('current_resource') == resource_id and 
            ...
            if (workspace_id in self.active_connections and 
                ...
                for websocket in self.active_connections[workspace_id][user_id][:]:
                    try:
                        ...
                        logger.error(f"Error sending resource message to user {user_id}: {e}")
                        self.active_connections[workspace_id][user_id].remove(websocket)

    async def broadcast_presence_update(self, workspace_id: int, user_id: int, exclude_user: int = None):
        """Broadcast presence update to workspace"""
        if (workspace_id not in self.user_presence or 
            user_id not in self.user_presence[workspace_id]):
            return
...
async def websocket_collaboration_handler(
    ...
    try:
        ...
        # Connect user
        connected = await collaboration_manager.connect(websocket, user, workspace_id, db)
        if not connected:
            return
        ...
        try:
            ...
            while True:
                try:
                    ...
                    await collaboration_manager.send_personal_message(websocket, {
                        'type': 'error',
                        'data': {
