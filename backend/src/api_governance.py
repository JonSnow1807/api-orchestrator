...
class APIGovernance:
    ...
    async def scan_api(
        ...
        for rule in self.rules + self.custom_rules:
            ...
            if compliance_frameworks:
                if not any(cf in rule.compliance_frameworks for cf in compliance_frameworks):
                    continue
    ...
    async def _validate_rule(
        ...
        if "openapi" in api_spec or "swagger" in api_spec:
            violations.extend(
                await self._validate_openapi_rule(rule, api_spec)
            )
        
        # Custom validation logic
        if rule.validator:
            custom_violations = await self._run_custom_validator(
                rule, api_spec
            )
    ...
    async def _validate_openapi_rule(
        ...
        for path, methods in paths.items():
            ...
            if rule.id == "naming-001":
                ...
                for part in parts:
                    if not part.startswith("{") and not re.match(r"^[a-z]+(-[a-z]+)*$", part):
                        violations.append(RuleViolation(
                            rule_id=rule.id,
                            rule_name=rule.name,
                            type=rule.type,
            ...
            for method, operation in methods.items():
                    ...
                    for status_code, response in responses.items():
                        if "example" not in response and "examples" not in response:
                            violations.append(RuleViolation(
                                ...
                                location={"path": path, "method": method, "status": status_code},
                ...
                if rule.id == "perf-001" and method == "get":
                    ...
                    if not has_pagination and "list" in operation.get("operationId", "").lower():
                        violations.append(RuleViolation(
                            rule_id=rule.id,
                            rule_name=rule.name,
                            type=rule.type,
    ...
    def _check_framework_compliance(
        ...
        violated_rules = {v.rule_id for v in violations if v.severity == RuleSeverity.ERROR}
