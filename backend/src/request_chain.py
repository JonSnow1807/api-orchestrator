...
class ChainedRequest(BaseModel):
    ...
    extract_from_response: Dict[str, str] = Field(
        default_factory=dict
    )  # Extract values from response
    conditions: List[Dict[str, Any]] = Field(
        default_factory=list
    )  # Conditions to run this request
...
class ChainExecutor:
    ...
    async def execute_chain(
        self, chain: RequestChain, initial_variables: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
            ...
            try:
                ...
                if request.extract_from_response:
                    extracted = self._extract_values(
                        response, request.extract_from_response
                    )
        ...
        return {
            ...
            "context": {
                "variables": {
                    ...
                    if k not in ["responses", "extracted"]
                },
    ...
    def _extract_values(
        self, response: Dict, extract_config: Dict[str, str]
    ) -> Dict[str, Any]:
    ...
    def _build_dependency_graph(
        self, requests: List[ChainedRequest]
    ) -> Dict[str, List[str]]:
    ...
    def _topological_sort(
        self, requests: List[ChainedRequest], graph: Dict[str, List[str]]
    ) -> List[ChainedRequest]:
