...
class RealTimeMonitor:
            ...
            try:
                ...
                if collab_status:
                    ...
                    if total_agents > 0:
                        ...
                        await self._record_metric("agent_utilization", agent_utilization, "%")
                    ...
                    await self._record_metric("completed_tasks", completed_tasks, "count")

                    # Collaboration metrics
                    metrics = collab_status.get("collaboration_metrics", {})
                    if metrics:
                        successful_collaborations = metrics.get("successful_collaborations", 0)
                        failed_collaborations = metrics.get("failed_collaborations", 0)
                        total_collaborations = successful_collaborations + failed_collaborations

                        if total_collaborations > 0:
                            failure_rate = (failed_collaborations / total_collaborations) * 100
                            await self._record_metric("agent_failure_rate", failure_rate, "%")
    ...
    async def _monitor_application_health(self):
        ...
        while self.is_monitoring:
            try:
                ...
                await self._record_metric("response_time", simulated_response_time, "ms")
                ...
                await self._record_metric("db_connections", random.randint(1, 10), "count")
    ...
    async def _record_metric(self, name: str, value: float, unit: str):
        ...
        if name in self.thresholds:
            ...
            if value >= critical_threshold:
                ...
                message = f"{name} is critical: {value}{unit} >= {critical_threshold}{unit}"
                await self._create_alert(name, AlertSeverity.CRITICAL, message)
            elif value >= warning_threshold:
                status = HealthStatus.WARNING
                message = f"{name} is warning: {value}{unit} >= {warning_threshold}{unit}"
    ...
    async def _create_alert(self, component: str, severity: AlertSeverity, message: str):
                    ...
                    if not alert.resolved:
                        ...
                        if time_since_alert > timedelta(minutes=5):
                            ...
                            if metric and metric.status == HealthStatus.HEALTHY:
                                ...
                                self.logger.info(f"✅ RESOLVED: Alert {alert_id} for {alert.component}")
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        critical_count = sum(1 for m in self.health_checks.values() if m.status == HealthStatus.CRITICAL)
        warning_count = sum(1 for m in self.health_checks.values() if m.status == HealthStatus.WARNING)
        ...
        return {
            ...
            "metric_summary": {
                ...
                "healthy": len([m for m in self.health_checks.values() if m.status == HealthStatus.HEALTHY]),
    ...
    async def save_health_report(self, filepath: str = None):
        ...
        for metric_name in self.metrics_history.keys():
            health_data["metric_history"][metric_name] = self.get_metric_history(metric_name, 50)
